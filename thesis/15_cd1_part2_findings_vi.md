...
*⚠️ **Ghi chú dữ liệu — CHƯA XÁC MINH (provisional estimate)**: Số liệu tỷ lệ phần trăm trong bảng này là **ước tính sơ bộ** dựa trên tổng hợp Carboni et al. (2024/2025) và World Bank (2024) cho các vùng WB rộng — chưa được tính toán trực tiếp từ biến `b7a` và `b4` của pool WBES 101.185 doanh nghiệp trong chuyên đề này. **Cần thực hiện**: chạy `tabstat b7a b4, by(icrv_regime) stat(mean n)` trên dataset CĐ1 để thay thế bằng số liệu chính xác trước khi nộp Hội đồng. Bảng 6.1.1 (§6.1) cung cấp số liệu vùng WB tham khảo nhưng không phân theo ICRV typology của chuyên đề này.*

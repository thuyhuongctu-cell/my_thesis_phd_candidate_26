...
|---|---|---|---|---|---|
| **Advanced — innovation-driven** *(SG, HK, TWN, ISR)* | ~8 | ~4.220 | **1,03** | **11,9** | **3,4** |
| **Advanced — resource-driven** *(SAU, QAT, BHR, BRN)* | ~5 | ~1.932 | **0,49** | **5,8** | **2,1** |
...
| **SIDS Thái Bình Dương (v3.1: gồm Kiribati 2025)** | **10** | **1.097** | **1,32** | **28,8** | **5,6** |

*Ghi chú: Advanced tách 2 phân nhóm con ở v3.1 — tỷ số phân tán Singapore (1,03) so với Vùng Vịnh Saudi/Qatar (0,49) ≈ **2,1 lần**. P90/P10 tính theo phương pháp weighted average per-country (log d2/l1, winsorize 1/99 trong từng country cluster, sóng gần nhất): Advanced innovation-driven từ SG+HK+TWN+ISR (n=1.668); Advanced resource-driven từ SAU+QAT+BHR+BRN (n=1.696, Saudi/Qatar chiếm 88% trọng số); SIDS từ 6 quốc gia (n=698). SIDS row v3.1 với Kiribati 2025 (n=150, sd log 1,48).*
...
Spec 1 (full coverage 2009–2025) dùng biến nhị phân duy nhất (`website` Y/N) đại diện cho "Năng lực số (DAI)". Trong khi đó TCI được đo bằng nhiều thành phần vững chắc (R&D + ISO multi-component aspirational). **Hậu quả phân tích**: hệ số -0,129 cho DAI ở Advanced trong Spec 1 gợi ý "số hóa làm giảm năng suất ở Singapore/Hàn Quốc/Hong Kong" — nhưng đây là **ảo ảnh thống kê** do đo lường sơ sài, không phải thực tế kinh tế. Như tác giả lưu ý (ghi chú nội bộ, 2026): *"Giống như cố đánh giá hiệu suất của mạng lưới logistic AI hiện đại nhưng dùng bản đồ trạm điện tín thế kỷ 19."*
...
(a) **Rào cản 1 — Hạn chế tín dụng**: **30,6% doanh nghiệp Frontier** và **33,8% doanh nghiệp Emerging** xem **hạn chế tiếp cận tín dụng (credit constraint)** là rào cản từ vừa trở lên (k30 ≥ 2 — moderate/major/very severe; tính toán của tác giả từ WBES sóng gần nhất, Frontier n=4.216, Emerging n=17.106). Cao nhất: Nepal 51,2%, Sri Lanka 75,3%, Cambodia 43,0%; thấp nhất: Uzbekistan 12,4%, Bangladesh 22,2%. Pattern phù hợp giả thuyết **khoảng trống thể chế (institutional voids)** của Khanna & Palepu (2010): các thị trường đang phát triển có hệ thống tài chính kém phát triển → doanh nghiệp khó tiếp cận tín dụng chính thức. Liên kết với §4.4 *Số hóa tài chính (Financial digitalization)* — công nghệ tài chính (fintech) có thể giảm chi phí giao dịch nhưng chưa thay thế được kênh ngân hàng truyền thống ở SME.
...
**Bảng 4.5.6**. *Tỷ lệ nữ quản lý cấp cao (female top manager) và nữ sở hữu — theo phân nhóm con ICRV (tính toán trực tiếp từ WBES).*

| Phân nhóm con | % Nữ Top Manager (b7a) | N (b7a) | % Nữ Majority Owner (b4) | N (b4) | Quốc gia đại diện |
|---|---|---|---|---|---|
| **Advanced — innovation-driven** | **20,2%** | 623 | **35,9%** | 622 | Singapore 2023 |
| **Advanced — resource-driven** | **1,7%** | 1.632 | **5,0%** | 1.629 | Saudi Arabia 1,6%; Qatar 1,0%; Bahrain 4,7% |
| **Upper-middle** | **26,8%** | 6.552 | **41,0%** | 6.483 | Thailand 51,2% (cao nhất); China 17,1%; Kazakhstan 26,0% |
| **Emerging** | **13,9%** | 17.289 | **19,7%** | 17.263 | Philippines 33,7%; Lao PDR 47,7%; India 7,2%; Bangladesh 3,0% |
| **Frontier** | **12,7%** | 4.229 | **17,7%** | 4.231 | Myanmar 30,1%; Timor-Leste 30,0%; Pakistan 3,6%; Tajikistan 8,0% |
| **SIDS Thái Bình Dương** | **29,2%** | 864 | **42,4%** | 853 | Samoa 47,8%; Tonga 30,0%; Fiji 18,5%; Solomon Islands 20,7% |

*Nguồn: Tính toán của tác giả từ dữ liệu WBES (World Bank Enterprise Surveys), 33 nền kinh tế, sóng gần nhất mỗi quốc gia (2016–2025). Phân nhóm ICRV theo khung CĐ1. Biến `b7a`: 1 = Yes (nữ quản lý cấp cao), 2 = No; biến `b4`: 1 = Yes (nữ sở hữu đa số), 2 = No; giá trị âm (−9, −7) bị loại trước khi tính trung bình. N tổng hợp 6 nhóm = 31.189 doanh nghiệp.*

**Ba phát hiện nổi bật**:

(a) **Phân tán nội bộ trong Emerging — khoảng cách SAS vs EAP**: Nhóm Emerging ghi nhận 13,9% nữ TMT bình quân nhưng che giấu khoảng cách lớn giữa SAS và EAP: India (7,2%, n=9.365) và Bangladesh (3,0%) kéo mức bình quân xuống, trong khi các nền kinh tế EAP trong cùng nhóm (Lao PDR 47,7%; Philippines 33,7%; Cambodia 31,6%; Indonesia 25,0%; Vietnam 20,2%) vẫn duy trì mức đại diện cao hơn đáng kể. Carboni et al. cho thấy mối quan hệ female top manager × ESG phụ thuộc mạnh vào chất lượng thể chế và công nghệ — cần kiểm soát quy mô, ngành và ICRV regime khi so sánh trong nhóm.

(b) **Vùng Vịnh — gap lớn nhất, được xác nhận bằng dữ liệu**: Advanced resource-driven (Saudi Arabia 1,6%; Qatar 1,0%) ghi nhận tỷ lệ nữ TMT thấp nhất toàn pool (1,7%), thấp hơn đáng kể so với ước tính sơ bộ trước đó (3–8%). Đây là boundary condition cho mọi phân tích gender × productivity: kết quả từ Vùng Vịnh không thể suy rộng sang Advanced innovation-driven.

(c) **Frontier — phân tán nội bộ lớn nhất trong nhóm**: Myanmar (30,1%) và Timor-Leste (30,0%) ghi nhận mức đại diện ngang Upper-middle, trong khi Pakistan (3,6%), Tajikistan (8,0%), Turkmenistan (6,4%) ở mức rất thấp — minh họa tại sao ICRV phân nhóm Frontier thành tiểu nhóm thay vì gộp chung và cảnh báo rủi ro ecological fallacy khi dùng mức bình quân nhóm.
...
| SIDS Thái Bình Dương | **+33,8** | **−2,4** | **−4,4** | N/A† | **−12,0** |

Nhảy vọt số (digital leapfrog): website tăng +20–43 đpt ở Frontier, Emerging và SIDS Thái Bình Dương — phù hợp luận điểm digital leapfrog của Banalieva & Dhanaraj (2019). *(†R&D: biến d7 không có trong schema BREADY 2018–2025 của SIDS — không thể tính Δ từ nguồn nhất quán; dữ liệu period 1: Fiji 2009 = 0,6%, Vanuatu 2009 = 0,8%.)*
...
**Hàm ý cho CĐ2 và luận án**:
- **(1) CĐ1 v3.x** sẽ dùng "Hối lộ" (Bribery) và "Tham ô" (Graft) thay vì "tham nhũng" tổng quát khi cite số liệu cụ thể từ WBES.
- **(2) CĐ2 specification** sẽ có **3 biến phân biệt**: `bribery_incidence_pct` (a-1), `bribery_depth_pct` (a-2), `graft_index_pct` (b-1) từ WBES question j7a-j7f → kiểm tra hệ số khác nhau xuyên 8 phân nhóm con thể chế.
- **(3) Liên kết với §4.5.5 rào cản #3** (cạnh tranh phi chính thức): DN phi chính thức tránh thuế nhưng cũng có thể **né tránh hối lộ (bypass bribery)** do hoạt động ngoài hệ thống chính thức.
- **(4) Liên kết với khung phân tích cấp ngành §4.8**: ngành Xây dựng (cường độ hợp đồng cao) thường có Bribery Depth cao; ngành Khai khoáng/Du lịch (tỷ trọng MNE cao) có Bribery Incidence cao nhưng Depth thấp.

### 4.8 Khung phân tích cấp ngành (industry-level) — kế hoạch CĐ2

> **Mở rộng v3.4 (07/05/2026)**: Bảng 4.8.1 thêm cột "Dynamism profile (Kafouros 2023)".
...
|---|---|---|---|---|
| Manufacturing — high-tech | C (20–21, 26–30) | Thâm dụng vốn, định hướng xuất khẩu | FSTS cao; R&D cao; ISO cao | **High technological dynamism** |
| Manufacturing — low-tech | C (10–18, 22–25, 31–33) | Thâm dụng lao động | FSTS trung bình; R&D thấp | **Low technological dynamism** |
| Wholesale/retail | G (45–47) | Thâm dụng lao động, định hướng trong nước | FSTS thấp; website cao; FDI thấp | **High market dynamism** |
...
| Construction | F (41–43) | Theo dự án, định hướng trong nước | FSTS thấp; FDI ở GCC | **Low dynamism** |
...
**Lý giải lý thuyết — Kafouros et al. (2023)**: chất lượng thể chế tương tác hai chiều với động học ngành — khuếch đại tích cực ở high-tech dynamism, suy yếu ở high-market dynamism.

**6 hàm ý phương pháp luận**: (a) Industry FE 9 ngành; (b) Manufacturing-only subsample; (c) Tourism separation cho SIDS; (d) Construction subsample test Vùng Vịnh; (e) ICT exclusion test cho DAI; (f) Test interaction `tech_dynamism × institutional_quality`.
...
Khai báo biến giả `Post_BREADY_2024` = 1 cho mọi observation thuộc đợt khảo sát 2025+ (BREADY schema), = 0 cho 2009–2024. Hấp thụ schema effect tĩnh.

#### 4.11.2 Đề xuất 3b — Mô hình neo (anchor model)

Quy trình 3 bước: (i) chạy hồi quy với data đến 2024 → anchor coefficients; (ii) khóa hệ số; (iii) re-run với full pool 2009-2025, so sánh Wald test + LR test. Robustness check thứ 6 cho CĐ2.

#### 4.11.3 Đề xuất 3c — Tách 2025 thành "Panel hậu đại dịch độc lập"

Dành riêng sub-section trong CĐ2 xử lý 2025 như out-of-sample validation panel cho kỷ nguyên hậu COVID + AI. Ba bước: chạy mô hình chính 2009-2024 → test trên 2025-only → báo cáo coefficient stability.

#### 4.11.4 Hệ thống triple-defense chống đứt gãy schema

**Bảng 4.11**. *Hệ thống ba lớp phòng vệ (triple-defense) chống đứt gãy schema BREADY 2025.*

| Đề xuất | Tác dụng | Robustness check # |
|---|---|---|
| 3a Biến giả Post_BREADY_2024 | Hấp thụ schema effect tĩnh | (parametric correction) |
| 3b Anchor model | Test stability hệ số | #6 |
| 3c Panel hậu đại dịch độc lập | Out-of-sample validation | #7 panel test |

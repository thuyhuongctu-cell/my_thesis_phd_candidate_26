  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}\small
        ...
        \item Mở rộng ra \hl{49 nền kinh tế} châu Á--TBD (gồm Nhật Bản).
...
\begin{frame}{Khung khái niệm tích hợp đa tầng CDCM (Hình 2.1)}
  \begin{columns}[T]
    ...
    \begin{tikzpicture}[
      ...
      \node[mod,fill=gray!10,draw=gray,below=1.7cm of $(i)!0.5!(p)$] (fip) {Điều kiện biên FIP (H1b) tại SIDS\\$\Rightarrow$ chữ U ngược tan rã};
    ...
    \begin{column}{0.45\textwidth}\small
      ...
      \kw{ICRV} (Institutional Context Regime Variation) --- taxonomy thể chế \hl{6 nhóm} cho 49 nền:
...
\begin{frame}{Hệ giả thuyết H1--H6 (+ H1b, H1c)}
  \begin{itemize}\small
    ...
    \item \kw{H1b (FIP)} --- Tại SIDS (Nhóm VI), khi cả ba điều kiện cấu trúc tối thiểu bị vi phạm, dạng chữ U ngược \hl{tan rã} (mất cả độ cong lẫn độ dốc có ý nghĩa); quan hệ nghiêng về dạng âm đơn điệu như \emph{trường hợp giới hạn} --- \emph{gánh nặng quốc tế hóa bắt buộc}.
...
\begin{frame}{Thiết kế hỗn hợp tổng hợp--thực nghiệm (mixed)}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      \textbf{Dữ liệu WBES}
      \begin{itemize}\small
        \item Mẫu phân tích: \hl{87.987 doanh nghiệp / 49 nền kinh tế} (2003--2025); mẫu hồi quy chính $N=80.373$.
...
\begin{frame}{Kết quả 1: Chữ U ngược \& bức tranh ba vùng theo ICRV}
  \begin{columns}[T]
    ...
    Mô hình toàn mẫu P7 (49 nền, two-way FE):
    \[ \text{TP(M5)} = \hl{43{,}6\%}\ \text{FSTS} \quad (p_{\text{LM}}<{,}001) \]
    Mô hình rút gọn M2 ($N=80.373$): $\text{TP}=51{,}5\%$. Hệ số bậc hai âm có ý nghĩa ở cả hai.
    \vspace{0.3em}
    ...
    \begin{itemize}\scriptsize
      ...
      \item Dưới (Emerging/SIDS): cấu trúc \emph{tan rã}; FIP là trường hợp giới hạn lý thuyết.
    \end{itemize}
...
\begin{frame}{Kết quả 2: Gánh nặng quốc tế hóa bắt buộc (FIP) tại SIDS}
  \begin{columns}[T]
    ...
    Tại các nền kinh tế Pacific SIDS (Nhóm VI, bảy nền; loại Timor-Leste theo phân loại WB/UN), phát hiện \emph{vững} trên mẫu đầy đủ ($N=1.450$, bảy cụm) là \hl{sự tan rã của chữ U ngược}:
    ...
    \small \hl{FIP} là cấu trúc lý thuyết \emph{mới} --- lần đầu được định danh trong tài liệu I--P như \emph{trường hợp giới hạn} của sự tan rã: doanh nghiệp SIDS xuất khẩu \emph{để sinh tồn}, không phải để khai thác lợi thế cạnh tranh. Ủng hộ H1b theo hướng tan rã.
    \end{block}
    \centering
    \scalebox{0.85}{
    \begin{tikzpicture}
...
\begin{frame}{Tổng hợp kết quả kiểm định H1--H6 / H1b / H1c}
  ...
  \begin{tabular}{@{}p{1.6cm}p{4.3cm}p{6.0cm}@{}}
    ...
    H1b (FIP) & SIDS: chữ U ngược tan rã; FIP là trường hợp giới hạn & \kw{Ủng hộ} (mẫu đầy đủ: độ cong \& độ dốc n.s.; $-1{,}339$ chỉ ở bản dựng ba cụm) \\
...
\begin{frame}{Đóng góp lý thuyết}
  \begin{enumerate}
    ...
    \item \kw{Construct mới FIP} (Forced Internationalization Penalty) --- lần đầu định danh trường hợp giới hạn nơi chữ U ngược tan rã tại điều kiện biên cực đoan (SIDS), biện minh bằng ba điều kiện cấu trúc.
    \item \kw{Taxonomy ICRV} --- khung phân loại thể chế 6 nhóm cho 49 nền, công cụ tái sử dụng cho nghiên cứu I--P tương lai.
...
\begin{frame}{Hạn chế và hướng nghiên cứu tương lai}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}\small
        \item Nhật Bản (P10) đã tích hợp vào khung 49 nền; bước tiếp là đối chứng do-file Stata và bổ sung các sóng 2024--2026.
...
\begin{frame}{Kết luận}
  \begin{enumerate}
    ...
    \item Tại \kw{SIDS}, dạng chữ U ngược tan rã; \hl{FIP} là trường hợp giới hạn lý thuyết --- construct mới của luận án.
  ...
  \end{enumerate}
  \vspace{0.3em}
  \centering\small Mẫu phân tích: \hl{87.987 doanh nghiệp / 49 nền kinh tế} (WBES, 2003--2025; hồi quy chính $N=80.373$) --- meta-analysis $k=238$.
\end{frame}

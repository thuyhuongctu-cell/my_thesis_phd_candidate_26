...
%  "Quốc tế hóa và hiệu quả hoạt động kinh doanh của doanh nghiệp tại châu Á:
...
% --------- Thông tin tiêu đề ----------
\title[Quốc tế hóa \& hiệu quả DN tại châu Á]{Quốc tế hóa và hiệu quả hoạt động kinh doanh của doanh nghiệp tại châu Á}
...
\begin{frame}{Câu hỏi và mục tiêu nghiên cứu}
  \textbf{Câu hỏi trung tâm.} Quốc tế hóa ảnh hưởng đến hiệu quả hoạt động kinh doanh của doanh nghiệp ở châu Á \emph{như thế nào}, và \emph{điều kiện nào} --- thể chế, năng lực số, đặc điểm nhà quản trị --- giải thích sự dị biệt cao đó?
  \vspace{0.3em}
  \begin{columns}[T]
...
\begin{frame}{Khung khái niệm tích hợp đa tầng CDCM (Hình 2.1)}
  \begin{columns}[T]
    ...
    \begin{tikzpicture}[
      ...
      \node[mod,fill=gray!10,draw=gray,below=1.7cm of $(i)!0.5!(p)$] (fip) {Điều kiện biên FIP (H1b) tại SIDS\\$\Rightarrow$ chữ U ngược mất cấu trúc};
...
\begin{frame}{Hệ giả thuyết H1--H6 (+ H1b, H1c)}
  \begin{itemize}\small
    ...
    \item \kw{H1b (FIP)} --- Tại SIDS (Nhóm VI), khi cả ba điều kiện cấu trúc tối thiểu bị vi phạm, dạng chữ U ngược \hl{mất cấu trúc} (mất cả độ cong lẫn độ dốc có ý nghĩa); quan hệ nghiêng về dạng âm đơn điệu như \emph{trường hợp giới hạn} --- \emph{gánh nặng quốc tế hóa bắt buộc}.
    ...
    \item \kw{H5} --- Thể chế (ICRV) điều tiết phổ thể chế: dương lớn ở Nhóm I $\to$ âm ở Emerging/SIDS.
...
% ===================== SLIDE 8: KẾT QUẢ — CHỮ U NGƯỢC + PHỔ =====================
...
\begin{frame}{Kết quả 1: Chữ U ngược \& bức tranh ba vùng theo ICRV}
  \begin{columns}[T]
    ...
    \kw{Bức tranh ba vùng theo ICRV} (không còn phổ thể chế đơn điệu):
    \begin{itemize}\scriptsize
      ...
      \item Dưới (Emerging/SIDS): cấu trúc \emph{mất cấu trúc}; FIP là trường hợp giới hạn lý thuyết.
    \end{itemize}
...
\begin{frame}{Kết quả 2: Gánh nặng quốc tế hóa bắt buộc (FIP) tại SIDS}
  \begin{columns}[T]
    ...
    Tại các nền kinh tế Pacific SIDS (Nhóm VI, bảy nền; loại Timor-Leste theo phân loại WB/UN), phát hiện \emph{vững} trên mẫu đầy đủ ($N=1.450$, bảy cụm) là \hl{việc chữ U ngược mất cấu trúc}:
    \[ \hat\beta(\text{FSTS})=-0{,}085\ (p_{wild}={,}66);\ \ \hat\beta(\text{FSTS}^2)=+0{,}696\ (p_{wild}={,}082) \]
    \begin{itemize}
      \item Cả độ cong lẫn độ dốc \emph{không} có ý nghĩa $\Rightarrow$ \hl{không có điểm uốn, không có chữ U ngược}.
      \item Hệ số âm mạnh $-1{,}339$ ($p<{,}001$) chỉ xuất hiện trên bản dựng dữ liệu hạn chế \emph{ba cụm}; mang tính minh họa và nhạy cảm với phiên bản dữ liệu.
    \end{itemize}
    ...
    \small \hl{FIP} là cấu trúc lý thuyết \emph{mới} --- lần đầu được định danh trong tài liệu I--P như \emph{trường hợp giới hạn} của sự mất cấu trúc: doanh nghiệp SIDS xuất khẩu \emph{để sinh tồn}, không phải để khai thác lợi thế cạnh tranh. Ủng hộ H1b theo hướng mất cấu trúc.
    \end{block}
    \centering
    \scalebox{0.85}{
    \begin{tikzpicture}
...
\begin{frame}{Tổng hợp kết quả kiểm định H1--H6 / H1b / H1c}
  ...
  \begin{tabular}{@{}p{1.6cm}p{4.3cm}p{6.0cm}@{}}
    ...
    H1b (FIP) & SIDS: chữ U ngược mất cấu trúc; FIP là trường hợp giới hạn & \kw{Ủng hộ} (mẫu đầy đủ: độ cong \& độ dốc n.s.; $-1{,}339$ chỉ ở bản dựng ba cụm) \\
    ...
    H5 (ICRV phổ thể chế) & Thể chế điều tiết phổ thể chế & Xác nhận \emph{một phần}: $Q_M=17{,}35^{**}$; phổ thể chế TP từ P7 \\
...
\begin{frame}{Đóng góp lý thuyết}
  \begin{enumerate}
    ...
    \item \kw{Construct mới FIP} (Forced Internationalization Penalty) --- lần đầu định danh trường hợp giới hạn nơi chữ U ngược mất cấu trúc tại điều kiện biên cực đoan (SIDS), biện minh bằng ba điều kiện cấu trúc.
...
\begin{frame}{Kết luận}
  \begin{enumerate}
    \item Quan hệ I--P ở châu Á có dạng \hl{chữ U ngược} nhưng định hình theo \emph{ba vùng thể chế}: sắc nét ở nhóm chuyển đổi (TP $\approx 43\%$), gần tuyến tính ở thể chế mạnh nhất (gồm Nhật Bản), mất cấu trúc ở thể chế yếu nhất.
    ...
    \item Tại \kw{SIDS}, dạng chữ U ngược mất cấu trúc; \hl{FIP} là trường hợp giới hạn lý thuyết --- construct mới của luận án.

...
% --------- Chọn engine: pdfLaTeX (mặc định) ----------
\usepackage[utf8]{inputenc}
\usepackage[T5]{fontenc}        % T5 = bảng mã font cho tiếng Việt
\usepackage[vietnamese]{babel}
% -----------------------------------------------------
%
% Nếu dùng XELATEX thay vì pdfLaTeX, hãy COMMENT 3 dòng trên và UNCOMMENT khối sau:
% \usepackage{fontspec}
% \usepackage{polyglossia}
% \setmainlanguage{vietnamese}
% \setmainfont{Latin Modern Roman}   % hoặc "Times New Roman", "DejaVu Serif"
%
% -----------------------------------------------------
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}\small
        ...
        \item Mở rộng ra \hl{49 nền kinh tế} châu Á--TBD.
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}\small
      ...
      \begin{itemize}\scriptsize
        \item I. Advanced innovation (SG, KR, TW)
...
\begin{frame}{Thiết kế hỗn hợp tổng hợp--thực nghiệm (mixed)}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      \textbf{Dữ liệu WBES}
      \begin{itemize}\small
        \item Mẫu phân tích: \hl{91.982 doanh nghiệp / 49 nền kinh tế} (2003--2025).
...
\begin{frame}{Kết quả 1: Chữ U ngược \& gradient turning point theo ICRV}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}\small
    Mô hình toàn mẫu P7 (two-way FE, $\text{adj}R^2={,}677$):
    \[ \text{TP(M5)} = \hl{40{,}0\%}\ \text{FSTS} \quad (p_{\text{LM}}<{,}001) \]
    Turning point ổn định trong dải \hl{33{,}8--40{,}0\%} qua M2--M9; mô hình ba chiều M11: $\text{TP}=34{,}6\%$.
    \vspace{0.3em}

    \kw{Gradient ICRV} (turning point dịch phải khi thể chế yếu đi):
    \[ \underbrace{\approx\hl{28\%}}_{\text{Nhóm I}} < \text{Upper-mid} < \underbrace{\approx\hl{55\%}}_{\text{Emerging/SIDS}} \]
    ...
    \begin{tikzpicture}[scale=1.0]
      ...
      % curve Nhom I (TP som ~28%)
      \draw[very thick,accent] plot[domain=0:4.6,samples=60] (\x,{2.6-0.55*(\x-1.4)*(\x-1.4)+0.6});
      \draw[dashed,accent] (1.4,0) -- (1.4,2.6+0.6);
      \node[accent,font=\scriptsize] at (1.4,-0.25) {28};
      % curve Emerging (TP muon ~55%)
      \draw[very thick,accent2] plot[domain=0:4.6,samples=60] (\x,{2.1-0.30*(\x-2.9)*(\x-2.9)+0.6});
      \draw[dashed,accent2] (2.9,0) -- (2.9,2.1+0.6);
      \node[accent2,font=\scriptsize] at (2.9,-0.25) {55};
      % SIDS monotone neg
      \draw[very thick,gray,dotted] plot[domain=0.2:4.6,samples=40] (\x,{1.7-0.42*\x});
      \node[accent,font=\scriptsize,anchor=west] at (3.2,3.0) {Nhóm I};
      \node[accent2,font=\scriptsize,anchor=west] at (3.2,2.55) {Emerging};
      \node[gray,font=\scriptsize,anchor=west] at (2.6,0.4) {SIDS (FIP)};
    \end{tikzpicture}}
    \\{\scriptsize Hình 4.1 (minh họa): họ đường cong I--P theo ICRV.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Hạn chế và hướng nghiên cứu tương lai}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}\small
        \item Ước lượng lại P7 trên mẫu mở rộng (đã bổ sung Japan-2025, các sóng 2024--2026).
...
\begin{frame}{Kết luận}
  \begin{enumerate}
    \item Quan hệ I--P ở châu Á có dạng \hl{chữ U ngược} nhưng \emph{điểm uốn không cố định}: giảm dần theo chất lượng thể chế (từ $\approx 55\%$ ở thể chế yếu xuống $\approx 28\%$ ở thể chế mạnh).
  ...
  \end{enumerate}
  \vspace{0.3em}
  \centering\small Mẫu phân tích: \hl{91.982 doanh nghiệp / 49 nền kinh tế} (WBES, 2003--2025) --- meta-analysis $k=238$.
\end{frame}

...
**Tổng thời gian:** 30–45 phút trên ScholarOne.
...
- **Files đã có:**
  ...
  - `p4/submission/mir_package/02_title_page.docx`
  - `p4/submission/mir_package/03_cover_letter.docx`
  - Figures: 3 PNG files
...
- **Files đã có:**
  ...
  - `p5/submission/ijoem_package/02_title_page.docx`
  - `p5/submission/ijoem_package/03_cover_letter.docx`
  - Figures: 4 PNG files
...
## PHẦN III — CHUYÊN ĐỀ (VIETNAMESE SPECIALIZED PAPERS)

### CD1 — Chuyên đề 1 🟡 CẦN CTU REVIEW
- **File hoàn chỉnh:** `chuyen_de/cd1/00_cd1_ctu_final_vi.md` (101K)
- **Word export:** `dist/chuyen_de_1/00_cd1_ctu_final_vi.docx` (56K)
...
4. Sau khi GVHD ký chấp thuận: nộp Khoa để xếp lịch bảo vệ chuyên đề

**Bảo vệ chuyên đề 1:** Theo quy định CTU, tổ chức dưới dạng seminar nội bộ (~30 phút trình bày, Q&A).

---

### CD2 — Chuyên đề 2 🟡 CẦN CTU REVIEW
- **File hoàn chỉnh:** `chuyen_de/cd2/00_cd2_ctu_final_vi.md` (126K)
- **Word export:** `dist/chuyen_de_1/00_cd2_ctu_final_vi.docx` (776K)
...
## PHẦN IV — LUẬN ÁN (5 CHƯƠNG TIẾNG VIỆT)

### Trạng thái tổng quan các chương
...
## PHẦN V — CÔNG VIỆC NCS PHẢI TỰ THỰC HIỆN (KHÔNG THỂ DÙNG AI)

### 5.1 P6 L2 Extraction — Việc quan trọng nhất còn lại

**Bối cảnh:** 435 papers đã được pre-screened và sẵn sàng trong `p6/tools/results/fulltext_to_extraction_tracker_v2.csv`. Cần đọc full-text và điền vào tracker.

**Hướng dẫn chi tiết NCS:**

**Bước 1: Setup môi trường làm việc**
```bash
# Tải full-text PDFs (đã có script)
python3 p6/tools/30_oa_check_and_download.py \
  --input p6/tools/results/fulltext_to_extraction_tracker_v2.csv \
  --output p6/tools/results/oa_check_20260520.csv \
  --pdfs p6/tools/pdfs/ \
  --email seranguyenct@gmail.com
```
Script sẽ tự động tải PDFs qua OpenAccess. ~70–75% papers có free access.

**Bước 2: Mở tracker trong Excel/Google Sheets**
- File: `p6/tools/results/fulltext_to_extraction_tracker_v2.csv`
- 435 rows × 58 cột
- Sắp xếp theo `extraction_priority`: bắt đầu với `1_DOI_FIRST` (310 papers) trước

**Bước 3: Với mỗi paper, điền 5 cột quan trọng nhất**
```
fulltext_screening_decision: Y / N / UNSURE
fulltext_screening_reason:   (vì sao exclude nếu N)
r:                           Pearson r (hoặc estimate từ β/t/F)
n:                           Sample size
is_estimated:                0 (direct r) / 1 (converted từ β/t/F)
```

**Bước 4: Công thức chuyển đổi (dùng khi không có r trực tiếp)**
```
Từ t-statistic:  r = t / sqrt(t² + df)
Từ F (df1=1):    r = sqrt(F / (F + df_error))
Từ β (OLS):      r ≈ β (Peterson & Brown, 2005, acceptable approximation)
```

**Bước 5: Codebook để kiểm tra ICRV và DPL**
- Codebook: `p6/tools/p6_extraction_codebook.md`
- ICRV: 1=Advanced, 2=Emerging, 3=Transition, 4=Resource-rich, 5=SIDS, 6=Frontier
- DPL: 1=Pre-digital (before 2000), 2=Early digital (2000–2009), 3=Platform era (2010+)

**Timeline thực tế:**
- Trung bình 10–15 phút/paper (đọc abstract + tìm statistics table)
- 310 papers DOI: ~52–77 giờ (tương đương 7–10 ngày full-time)
- 125 papers manual: +15–20 giờ thêm
- **Tổng: 4–6 tuần nếu làm 2–3 giờ/ngày**

---

### 5.2 Scopus Search — Cần campus network

**Bối cảnh:** WoS đã search (782 papers). Scopus là database thứ hai bắt buộc trong PRISMA.

**NCS cần làm (phải ở campus CTU hoặc dùng VPN CTU):**
1. Truy cập scopus.com qua mạng campus CTU
2. Copy query từ `p6/p6_wos_search_guide.md` (section Scopus query)
3. Export CSV tối đa 2,000 records/lần
4. Upload CSV vào repo: `p6/tools/results/scopus_export_YYYYMMDD.csv`
5. Chạy script parse: `python3 p6/tools/02_parse_scopus_export.py`

---

### 5.3 Inter-Coder Reliability — Cần người thứ 2

**Bối cảnh:** PRISMA yêu cầu 20% double-coded. File subsample đã có: `p6/tools/results/reliability_subsample_v7_20260519.csv`

**NCS cần làm:**
1. Nhờ 1 người (nghiên cứu sinh khác, GVHD, hoặc đồng nghiệp) code 20% subsample (~87 papers) độc lập
2. Sau khi cả hai hoàn thành, gửi 2 file cho AI để tính ICC và κ
3. Target: Cohen's κ ≥ 0.70 (categorical), ICC ≥ 0.80 (continuous r)

---

### 5.4 Nộp Luận án CTU — Quy trình hành chính
...
- Quyết định giao đề tài (QĐ 4769/QĐ-ĐHCT)
- Luận án đóng bìa mềm (4 quyển cho cấp Cơ sở, 7 quyển cho cấp Trường)
- Tóm tắt luận án (tiếng Việt và tiếng Anh) — ~24 trang A5
- Bản kê các bài báo đã công bố
- **Lưu ý:** P1 và P2 đã công bố = đủ điều kiện bảo vệ (CTU yêu cầu 2 bài ISI/Scopus)

---

## PHẦN VI — ROADMAP THEO TIMELINE

### Nếu mục tiêu bảo vệ cuối năm 2026:
...
Tháng 6–8/2026:
  🔴 L2 extraction P6 (435 papers)     — 4–6 tuần
  🔴 Scopus search (campus network)    — 1–2 ngày
  🔴 Nộp CD2 cho GVHD                 — 1 ngày
  🔴 Bảo vệ CD1 và CD2                — lịch Khoa
  🟡 Chỉnh sửa CD1/CD2 sau bảo vệ    — 1–2 tuần

Tháng 8–9/2026:
  🔴 Inter-coder reliability (P6)      — 2 tuần
  🟡 Final polish thesis Ch.1–5        — 1–2 tuần
  🔴 Bảo vệ cấp Cơ sở                 — lịch Khoa

Tháng 10–11/2026:
  🟡 Chỉnh sửa sau cấp Cơ sở          — 1–2 tuần
  🔴 Bảo vệ cấp Trường                 — lịch Khoa
  🟡 Chỉnh sửa cuối cùng              — 2–3 tuần

Tháng 12/2026:
  🔴 Nộp luận án chính thức            — deadline CTU
```

---

## PHẦN VII — TÓM TẮT MỨC ĐỘ HOÀN THIỆN

| Component | % Xong | Chặng cuối |
|-----------|--------|-----------|
| P1 (VEFR) | **100%** | Đã đăng |
| P2 (JFAR) | **100%** | Đã đăng |

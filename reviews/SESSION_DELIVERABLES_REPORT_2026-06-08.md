...
**Phạm vi phiên:** 15 commit (`5cc13b4` → `9c80bf8`); **95 file thay đổi** (+2.091 / −4.192 dòng — giảm ròng do cô đọng).
...
| **Khai báo AI — luận án** | Đối chiếu *Hướng dẫn trình bày LATS ĐHCT* (63 tr.): chuẩn dùng **Lời cam đoan**, không có mục khai báo AI ⇒ gỡ mục AI, giữ Lời cam đoan | `thesis/00_phan_dau_vi.md`, `dist/luan_an_ctu/…` |
| **Tự‑trích‑dẫn/kết hợp công trình** | Bổ sung khai báo: luận án phát triển từ **2 chuyên đề của chính NCS** (Quyết định giao chuyên đề) → trùng lặp hợp lệ, không phải đạo văn | `thesis/00_phan_dau_vi.md` (Lời cam đoan) |
| **Khai báo AI — P6 (NGHIÊM TRỌNG)** | Bằng chứng (extractor dùng Claude + log auto-extract) ⇒ sửa câu sai "extracted manually / no GenAI" thành **khai báo M‑AIDA (LLM) có PI kiểm chứng 100% + khóa dữ liệu** ở cả 4 package | `p6/submission/{jim,ibr,jwb,apjm}/01_manuscript_blinded.md` |
| **OSF — sửa tuyên bố sai** | Bỏ "OSF pre-registration *preceded* extraction / registered *prior to database access*" → **transparency registration** (corpus đã thu thập), chèn `[insert OSF DOI]` | 4 package P6 |
...
|---|---|---|
| **P6 cỡ mẫu lệch** (237/287 vs 238/288) | Đối chiếu dữ liệu gốc (`p6_study_database.csv`=288 effects/238 study; workbook "r=0.074") → chuẩn hoá toàn repo về **k=238/K=288** | JIM+APJM manuscripts, cover letter, README |
...
| Việc | Chi tiết |
|---|---|
| **Cơ sở** | Loại **Comoros** (Ấn Độ Dương) + **Timor-Leste** (ĐNÁ) → 7 đảo nhỏ TBD thật, khớp định nghĩa chuyên đề (không theo p-value) |
...
| **Viết lại** | 3 package P8 (jid/world_development/ejdr): abstract, mẫu, **Table 1/2/3**, mọi hệ số in-text; khung lại trung thực (bivariate/exporters yếu → dựa FE) |
| **Đồng bộ** | Abstract luận án + positioning (chín→bảy); footnote "dữ liệu cập nhật" ở CĐ1 |
| **Lưu** | Script + CSV kết quả 7 nước | `p8/replication/reanalysis_7pacific/` (5 file) |

## 4. Chuẩn hoá 6 hồ sơ nộp đầu (P3→JED, P4→MBR, P5→IJOEM, P6→JIM, P7→IBR, P8→JID)

| Việc | Chi tiết |
|---|---|
| **Abstract theo NXB** | Emerald (JED/MBR/IJOEM) → **cấu trúc** Purpose/Design/Findings/Originality; Elsevier (JIM/IBR) + Wiley (JID) → **1 đoạn** |
| **Cover letter — sửa lệch dữ liệu** | P5: bản cũ 4 đợt (2003–2024) → **2 đợt (2012/2024)** khớp manuscript; P6: 288/238 |
| **Em dash** | Gỡ sạch (0) toàn bộ 6 package (manuscript, cover, title, tables, supplementary, README) |
| **README** | Đồng bộ tiêu đề/số từ/checklist (vd P5 tiêu đề sai, P7 "structured"→1 đoạn) |
| **Harvard (Emerald)** | `, &`→`, and` trong TLTK; `&`→`and` trong trích dẫn có năm (giữ "&" tên tạp chí/R&D) — JED/MBR×2/CMS/IJOEM |

## 5. Cô đọng độ dài (đạt trần tạp chí)

| Paper | Trước → Sau | Giữ nguyên |
|---|---|---|
| **P6/APJM** | 13.467 → **8.351 từ** (≤10k); Phụ lục A–C → supplement | Results/Discussion/References |
| **P4/MBR** | 12.704 → **10.616 từ** (main ~9.100) | Mọi hệ số, bảng M0–M8, H1–H3 |
...
| Việc | Chi tiết | File |
|---|---|---|
| **Tự‑trích‑dẫn P7 §2.5** | Thêm **Phan, Đỗ & Phan (2020)** — bài của nhóm về kinh nghiệm/giới tính nhà quản trị điều tiết I–P (củng cố H4 + Đ.2.3) + **Nielsen (2010)** | P7 ibr/apjm/jibs |
| Luận án §2.5.5 | Thêm Phan (2020) vào H4 + nạp 2 reference vào danh mục | `thesis/chuong_2…`, `thesis/04_references_apa7.md` |
| **Bản đồ tích hợp** | Map 6 nguồn học thuật + báo cáo kinh tế (IMF Asia-Pacific, WB China/Indonesia/Mekong, Schulze meta…) → section đích, reference sẵn dán | `reviews/INTEGRATION_ATTACHED_SOURCES.md` |
...
| Sửa nhãn lỗi thời `cdai_score`: "Cultural Distance" → "country Digital Adoption Index 0–1" | `p6/tools/maida/backend/extractor.py` |
...
1. **P6:** chạy tìm kiếm WoS/Scopus chính thức (server không truy cập) → điền 29 `[TBD]` PRISMA.
2. **P6:** ICR — cần **coder thứ hai** mã hoá 20% mẫu → tính κ/ICC.
3. **OSF:** đăng ký thật → lấy DOI → dán vào 4 manuscript + file OSF.

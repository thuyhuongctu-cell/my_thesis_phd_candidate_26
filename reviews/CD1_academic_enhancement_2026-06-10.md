...
→ **Không có lỗi cấu trúc/khoa học chặn.** Các điểm dưới là **nâng cấp** (enhancement), không phải sửa lỗi.
...
- **Kruskal–Wallis** (hoặc ANOVA + Welch) cho LP/FSTS/website/FDI **xuyên 6 regime** + **post-hoc Dunn/Games-Howell** → khẳng định khác biệt có ý nghĩa.
...
- **Phân rã phương sai Theil/MLD** năng suất: **within-regime vs between-regime** (và within vs between country) → định lượng bao nhiêu % dị biệt là do thể chế (between) so với nội bộ (within).
...
- **Ma trận tương quan** giữa **tất cả** biến (hiện chỉ có Pearson I→P theo regime ở Bảng 2.3.8.1) → kiểm tra đa cộng + pattern liên biến.
...
So sánh năng suất/FSTS từ WBES với **nguồn vĩ mô độc lập** (national accounts, ADB Productivity, OECD, WB WDI) cho vài nước → xác nhận bức tranh WBES không lệch hệ thống.

### C2. **So sánh khung ICRV với phân loại sẵn có**
Đối chiếu 6 nhóm ICRV với **WB income groups / WGI clusters / Varieties of Capitalism (Hall & Soskice)** → chứng minh ICRV **không trùng lặp**, có giá trị phân biệt (đã có nguyên tắc "tính phân biệt" §2.3.2.1 nhưng chưa kiểm định thực nghiệm).

### C3. **Phân rã thành phần ngành (shift-share)**
Khác biệt năng suất giữa regime có phải do **cơ cấu ngành** (sector mix) hay **năng suất trong ngành**? Thêm shift-share / sector-fixed comparison → loại trừ "regime difference chỉ là sector composition".

### C4. **Tiểu-panel cân bằng cho phân tích thời gian**
Các bảng "Δ 2018-2025 vs 2009-2012" là **so sánh cohort chéo**, không panel. Bổ sung **tập con nước có ≥2 sóng** (FJI/PNG/VUT, VN, China...) + quỹ đạo within-economy → củng cố claim "nhảy vọt số" (hiện chỉ Mongolia có).

### C5. **Minh họa trực quan mạnh hơn** (giá trị cốt lõi của landscape paper)
- **Bản đồ 49 nước** tô màu theo regime ICRV.
- **Scatter FSTS × ln(LP)** toàn mẫu, tô màu regime + **LOWESS theo regime** → trực quan hóa I→P phi tuyến (đang mô tả bằng lời).
- **Heatmap 4 chiều hiệu quả × 6 regime** (z-score) → 1 hình tóm tắt toàn bộ.

---

## D. 🟢 TINH CHỈNH NHỎ
- Schema-break BREADY 2025: hệ thống phòng thủ 3 tầng **đề xuất** nhưng chưa **trình diễn** — thêm 1 bảng before/after cho chỉ số bị ảnh hưởng (Ấn Độ FSTS 7,7→2,7%) để minh chứng.
- Bối cảnh 2026 (xung đột Trung Đông, thuế quan Mỹ) ở §2.1: nêu rõ **chưa nằm trong data 2025** (caveat thời điểm).
- Chuyển §2.3.9.2 (5 khoảng trống) thành **bảng "khoảng trống → câu hỏi nghiên cứu CĐ2"** chính thức → nối logic sang CĐ2 mạch lạc hơn.

---

## E. LƯU Ý THỰC THI
Nhiều mục B1–B3, C1–C5 **cần chạy lại trên dữ liệu** (Kruskal/Theil/Table 1/correlation/scatter). Phần lớn biến có trong master `.dta` (LP, FSTS, website, FDI, exporter, TCI) → **tôi tính được khi re-lock** (cùng lúc có 43 raw `.dta` để khớp pipeline CĐ1). Riêng:
...
**Khuyến nghị thứ tự:** B4 + D (ngay) → B1/B2/B3 (khi có data) → C (publication round).

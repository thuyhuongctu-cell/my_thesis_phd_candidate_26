...
- **Khóa canonical** `data_wbes/analysis/CANONICAL_NUMBERS.md`. Xác minh thực nghiệm: P7 re-run GỒM Nhật Bản đến **50 nền/88.869/M2 81.022/M5 79.080/TP 43,6%**; "91.982/49" là build cũ superseded (bỏ Nhật chỉ còn 86.701). **Rối "49 vs 50" đã được giải quyết: đáp án là 50.**
- **Luận án** (Ch1–5 + Phụ lục A EN): đồng bộ 49 đến 50, 91.982 đến 88.869; sửa Mục 4.1.5 (Nhật ĐÃ vào P7); Phụ lục A cập nhật sơ đồ PRISMA + đánh dấu build cũ superseded. đến V3 xử lý xong cho luận án.
...
- **P7 manuscript (V1 XỬ LÝ XONG):** tái ước lượng đầy đủ ladder M1–M8 trên 50 nền (`scripts/p7_full_ladder.py`); hai mỏ neo tái lập chính xác (M2 81.022/51,5%; M4 79.080/43,6%). Viết lại toàn bộ manuscript. Hai sửa đổi bản chất từ re-estimation: (i) **DAI là level-shifter (+0,219***), KHÔNG uốn cong** (tương tác FSTS×DAI không ý nghĩa) — sửa luận điểm cũ; (ii) **nữ quản lý đảo dấu +0,185 đến −0,104***** (nhất quán P10/luận án); gradient 28 đến 55% đến **ba vùng** (Bảng 4 per-ICRV).
...
- **Item 8:** `scripts/verify_all.py` tái lập **14/14 PASS** + `REPRODUCIBILITY.md` (bản đồ claim đến code, quy trình hội đồng tự kiểm, AI-use disclosure, lập luận xuất xứ).
- **Item 5:** `osf/OSF_PROJECT_STRUCTURE.md` (layout OSF từng paper + nhãn registration đúng).
- **Item 4:** `p6/tools/maida/` đủ bộ citability (`CITATION.cff`, `.zenodo.json`, `LICENSE`, `CHANGELOG.md`, `PUBLISHING.md`) đến repo `thuyhuongctu-cell/maida-core` + Zenodo DOI.
...
| P3 Việt Nam | 8 | 8 | 7 | 8 | 6 | 7.4 | Có Đã sửa (LaTeX+TP+Mục ) |
| P4 Singapore | 8 | 7 | 8 | 9 | 8 | 8.0 | Có Đã sửa (TCI+TP) |
| P5 Trung Quốc | 8 | 8 | 8 | 8 | 8 | 8.0 | Có Đã sửa (N reconcile) |
| **P6 Meta-analysis** | 6 | 6 | 5 | 7 | 6 | 6.0 | **CHƯA HOÀN TẤT** |
| P7 Capstone 50 nền | 7 | 7 | 8 | 7 | 7 | 7.2 | Có Đã tái ước lượng + viết lại |
...
- **P3:** hỏng công thức `FSTS^{c} 2` (mã hóa squared term) khắp `:549-551, 608-610`; bảng TP mâu thuẫn (Table 2 thang 0,27–0,29 vs Table 4 thang 39–46%); mục 5.4 đến 5.6 thiếu 5.5.
...
- **Quy trình "freeze".** Sau mỗi lần chạy estimation, xuất bảng từ log đến cập nhật CANONICAL đến propagate. Không bao giờ để hai số khác nhau cùng tồn tại.
...
|---|---|---|---|
| **P9 Ấn Độ** | Management International Review | Q1/Q2 | Cross-section lặp (không panel) đến "tan biến ngưỡng" có thể do thành phần mẫu; thừa nhận thẳng + thêm kiểm định thành phần. **Nộp sớm nhất — mạnh nhất.** |
| **P10 Nhật Bản** | Asian Business & Management | Q2 | Một sóng cắt ngang, xuất khẩu thấp (16,8%) đến đóng góp mô tả sạch nhưng khiêm tốn. Đã sẵn sàng. |
| **P4 Singapore** | J. of Asian Business Studies (hạ từ MIR) | Q2 | Hiệu ứng DAI×FSTS² dựa vào đuôi mỏng (f²=0,018 < 0,02; exporters N=84) đến đóng khung "gợi ý, chưa xác quyết". |
| **P5 Trung Quốc** | IJoEM | Q2 | Đóng góp là "không bác bỏ" (độ bền ngưỡng) đến cần phân tích công suất + kiểm định tương đương (equivalence test). |
| **P3 Việt Nam** | JABS/IJoEM (hạ từ APJM) | Q2 | Sửa hỏng công thức + inverted-U biến mất trong mẫu exporters-only (β₂ p=,66) đến giải thích biên tham gia. |
| **P8 SIDS/FIP** | J. of International Development (an toàn) hoặc World Development (tham vọng) | Q1/Q2 | Phê phán "chỉ là negative selection mẫu nhỏ" đến **dẫn bằng kết quả 9 nền N=1.469 (β=−0,510), KHÔNG dẫn N=26**; đóng khung "nhất quán với FIP, chưa loại trừ được selection". |
| **P7 Capstone** | International Business Review (không phải JIBS) | Q1 | JIBS đòi nhận diện nhân quả; WBES cross-section pooled là contingency mô tả đến IBR/JWB hợp hơn. Sửa số canon trước. |
...
**Trình tự khuyến nghị:** P9 đến P10 đến P4 đến P5 đến P3 đến P8 đến P7 đến P6. Công bố P9/P10 trước tạo "vết" đã-bình-duyệt củng cố luận án.
...
- Hồ sơ gồm: (1) Tờ khai đăng ký (mẫu COV); (2) 02 bản in mã nguồn (in 2 mặt, đánh số trang, đóng quyển — thường yêu cầu trọn bộ hoặc 25 trang đầu + 25 trang cuối tùy COV); (3) Giấy cam đoan tác giả tự sáng tạo; (4) CMND/CCCD; (5) nếu là tác phẩm trong khuôn khổ luận án/đơn vị đến giấy xác nhận quyền của VLUTE/CTU hoặc cam kết không tranh chấp. Lệ phí ~100.000đ/chương trình. Thời gian cấp ~15 ngày làm việc.
...
- **Cải thiện trước khi nộp:** (a) thêm header license (đề xuất license riêng "All rights reserved" cho bản nộp COV, hoặc MIT/Apache nếu muốn mở mã trên GitHub — KHÔNG dùng GPL nếu định thương mại hóa); (b) README mô tả kiến trúc + sơ đồ luồng (PDF đến trích xuất đến chuẩn hóa đến xuất CSV meta); (c) gắn version tag rõ ràng + commit hash để chứng minh mốc thời gian sáng tạo.

**Công bố lên GitHub của tác giả (củng cố chứng cứ năng lực lập trình):**
- Tạo repo công khai `m-aida` riêng (tách khỏi repo luận án), README chi tiết, ảnh chụp giao diện/luồng, ví dụ chạy được (sample PDF đến output CSV), CHANGELOG theo version.
- **Lịch sử commit là bằng chứng vàng** chống nghi ngờ "AI viết hộ": commit thường xuyên, nhỏ, có message mô tả quá trình suy nghĩ/sửa lỗi đến cho thấy quá trình phát triển thật của con người. Thêm `CITATION.cff` + Zenodo DOI (archive GitHub release đến Zenodo cấp DOI vĩnh viễn, trích dẫn được trong luận án).
...
- Mỗi OSF project tạo DOI đến trích dẫn chéo trong paper ("Data and code: osf.io/xxxx") đến tăng điểm reproducibility với reviewer Q1.
...
- **P1 (Vietnam Economic & Financial Review 2026, heterogeneity emerging Asia):** *Ưu:* đã công bố, đặt nền mô tả; *Nhược:* tạp chí trong nước hạng thấp đến đóng góp học thuật quốc tế hạn chế. *Khắc phục:* dùng làm "vết công bố" baseline, không kỳ vọng impact.
...
- **Meta 2025 (ICBEF 2024 proceedings, k=113):** *Ưu:* nền tảng meta cho P6, mốc baseline; *Nhược:* proceedings hội nghị, k=113 nhỏ so với k=238 hiện tại đến có thể bị hỏi vì sao k nhảy. *Khắc phục:* nêu rõ P6 là MỞ RỘNG có hệ thống (1982–2026) của baseline ICBEF, ghi rõ phương pháp bổ sung studies.
...
- **P6 Meta:** *Ưu:* thiết kế ba tầng đúng chuẩn, battery publication-bias đầy đủ; *Nhược:* CHƯA hoàn tất (TBD), prereg sai mốc, Q_M hai giá trị. *Khắc phục:* hoàn tất thật + đóng băng số. **Nhược phải chấp nhận:** ba moderator mới đều null đến đóng khung "null là phát hiện" (pub-bias gánh bài báo).
- **P7 Capstone:** *Ưu:* mẫu lớn nhất, tổng hợp CDCM+ICRV; *Nhược:* số lệch canon, không nhận diện nhân quả ở quy mô. *Khắc phục:* đồng bộ canon; hạ JIBS đến IBR. **Nhược phải chấp nhận:** WBES cross-section không cho nhân quả mạnh — đóng khung contingency mô tả.
...
**Có — khung lý thuyết là điểm mạnh nhất của luận án.** Tích hợp 5 lớp (Uppsala + RBV + thể chế + thượng tầng quản trị + lăng kính số) thành CDCM ba tầng là mạch lạc; suy luận FIP từ ba điều kiện cấu trúc bị vi phạm đồng thời là đoạn lý thuyết chặt nhất (`chuong_2:174-180`). Chuỗi CDCM đến giả thuyết đến kết quả đến thảo luận nhìn chung vững.
...
- **H4 (nhà quản trị) chưa được dữ liệu ủng hộ** như giả thuyết (hiệu ứng nữ ÂM, tương tác null) đến nên hạ tầng "cá nhân" xuống kiểm soát, hoặc pre-register confound chọn-lọc-ngành.
- **H6 (thời gian)** bị P5 bác (ổn định) nhưng P9 xác nhận (tan biến) đến H6 thực ra điều kiện theo *độ lớn cú sốc*, là tinh chỉnh hậu nghiệm đến diễn đạt lại H6 cho đúng.
- **H3 (DAI đổi độ cong)** chủ yếu dựa Singapore underpowered (power ~16%) đến giảm trọng lượng suy diễn, hoặc dẫn bằng kết quả pool.
- **H5 gradient** chỉ P7 xác nhận (P6 5-regime không test được 6 nhóm) đến nói rõ tam giác hóa một phần, không overclaim.
...
2. **Notebook/nhật ký phân tích.** Lưu các bản nháp do-file, log chạy thất bại, ghi chú quyết định ("vì sao chọn cluster theo nền", "vì sao z-spec không tái lập FIP −1,339") đến chứng minh hiểu biết kinh tế lượng/meta thật sự.

...
|---|---|---|
| **D1** | Re-lock TOÀN BỘ CĐ1 về 49/96.415 (thay mọi bảng) **hay** giữ bản khóa CĐ1 + ghi chú dual-lock (đã có dòng 41, 228)? | NCS trước đã chọn **re-lock toàn bộ** đến kế hoạch này theo đó. CĐ1 là chuyên đề độc lập; re-lock là thay đổi lớn — xác nhận trước khi chạy. |
...
1. NCS push 44 raw `.dta` đến `data_wbes/raw_dta/` (danh sách trong `CD1_relock_status_2026-06-11.md`).
2. Tôi verify: đọc được, đếm nền/sóng, phát hiện schema lạ (ISBS/Micro/panel-zip cần giải nén).
3. Mở rộng pipeline nếu có schema chưa phủ (panel `.zip` đến giải nén; ISBS/Micro đến map biến riêng hoặc loại có ghi chú).

## PHASE 2 — Cổng hiệu chỉnh (CALIBRATION GATE)
1. Chạy pipeline trên nền tham chiếu (D3) đến đối chiếu sd/FSTS/% với số CĐ1 đã công bố.
2. **Gate:** sd + tỷ lệ chính khớp CĐ1 trong dung sai (±0,05 sd; ±2đpt tỷ lệ) đến PASS, đi tiếp. Nếu lệch đến soát lại trích biến/winsorize/yes-rate, KHÔNG chạy tiếp tới khi khớp.
   *(Đã validated sơ bộ: Advanced sd 1,123 vs ~1,03; Emerging 1,387 vs ~1,36.)*

## PHASE 3 — Chạy toàn bộ đến bảng tái lập
1. `python3 scripts/cd1_relock_descriptives.py` đến output per-group + bổ sung per-economy.
2. Sinh đúng cấu trúc các bảng CĐ1 (script tạo CSV; tôi format sang Markdown).

## PHASE 4 — Re-lock manuscript CĐ1 (ánh xạ bảng đến output)

| Phần CĐ1 | Hiện (bản khóa cũ) | Thay bằng (nhãn dữ liệu) |
|---|---|---|
| Abstract (d39,54), Mục 2.1 (d199,205,274), Mục 2.3.3 (d453) | 47 nền · 101.185 · 108 cặp | **49 nền · 96.415 phân loại** (mô tả) · 102 cặp |
| **Bảng 2.3.2.1** (d418) danh sách 6 nhóm + n | IV Emerging 47.803 / V Frontier 28.678 / III 16.693 / Adv 6.640 / SIDS 1.371 | **IV Lower_mid 50.926 / V Emerging 18.569 / III 17.905 / I 4.708 / II 2.269 / VI 2.038**; membership: BGD/PAK đến IV, LKA/JOR đến V |
...
| Mục 2.3.7.3/4/5 tiêu đề | n=3.077 / 4.889 / 42.278 | số đã sửa |
...
1. Thesis **Ch4 Bảng 4.2**: gỡ caveat "bản khóa CĐ1" đến dùng số tái lập (giờ cùng nguồn).
2. CĐ2: đã ở 49-frame — verify khớp.
3. Chạy `scripts/check-consistency.py` đến 0 issue.
...
## Rủi ro & giảm thiểu
- **Schema lạ (ISBS/Micro/panel-zip)** đến một số nền có thể thiếu biến đổi mới đến ghi chú "n/a", không bịa; nêu rõ độ phủ.
- **Hệ số Pearson (Bảng 2.3.8.1)** cần ma trận tương quan đầy đủ — nếu pipeline chưa xuất, để PENDING, không thay số cũ bằng số sai.
- **India 16MB** và nền lớn: nếu chỉ push được 1 phần đến re-lock PARTIAL, ghi rõ nền nào đã/chưa, KHÔNG suy số.
- **Thay đổi lớn ở chuyên đề đã nộp** đến mỗi bảng thay đều ghi nguồn pipeline + giữ crosswalk minh bạch.

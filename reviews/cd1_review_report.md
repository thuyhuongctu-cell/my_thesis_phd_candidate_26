...
**Vị trí**:
- `thesis/15` §4.4.5.1, Bảng 4.4.5.1, §6.1.2 Bảng 6.1.2: **n=2,958**
- `thesis/16` §5.3 và Bảng 5.1: **n=3,077**
...
**Hành động**: Thêm footnote tại §5.3: *"n=3,077 firms tổng cộng; n=2,958 firms có đầy đủ dữ liệu cho phân tích hồi quy (complete case) — xem §4.4.5.1."*

---

### C2 — 2025 cohort n: §1.1 nói 17,107; §4.1 và Bảng 4.10 nói 16,979

**Vị trí**:
- `thesis/14` §1.1: **"14 đợt khảo sát với 17.107 doanh nghiệp"**
- `thesis/15` §4.1: **"14 đợt khảo sát năm 2025 với 16.979 doanh nghiệp"**
...
→ Tổng hàng trong Bảng 4.10 **SAI** (16,979 ≠ 17,107); §1.1 **đúng** (17,107).

**Hành động**: Sửa §4.1 và tổng hàng Bảng 4.10 từ 16,979 → 17,107. Nếu Nepal (NPL, n=1,740) bị loại khỏi phân tích do schema, ghi footnote rõ ràng.

---

### C3 — Bảng 4.1 n_firms vs §4.1 pool breakdown: Frontier chênh 34%

**Vị trí**: `thesis/15` §4.1 text vs Bảng 4.1 column "n_firms"

| Sub-regime | §4.1 text | Bảng 4.1 | Δ | Δ% |
|---|---|---|---|---|
...
**Giải thích khả dĩ**: Bảng 4.1 đo firms có đủ dữ liệu năng suất (productivity-complete), còn §4.1 đo tổng pool. Nhưng **chênh 34% ở Frontier là bất thường**.
...
**Vị trí**:
- `thesis/15` §4.4.5.1 và `thesis/16` §5.1: **P3 = Singapore** (Mar et al., 2026 — MIR), **P4 = Việt Nam** (Đỗ & Phan, 2026 — APJM)
- `thesis/14` §1.5: **P3 = Singapore**, **P4 = Việt Nam**
- **File system trong repo**: `p3/p3_vietnam_en_clean.md` → P3=Việt Nam; `p4/p4_singapore_en_clean.md` → P4=Singapore
- **Build script comment** (`build_dist.sh`): "P3 = Vietnam (theo content)", "P4 = Singapore (theo content)"

→ Text luận án và file system **đảo ngược nhau hoàn toàn**.

**Hành động**: Giữ file system (P3=Vietnam, P4=Singapore) và sửa references trong thesis/14+15+16:
- `P3 (Singapore — MIR)` → `P4 (Singapore — MIR)`
- `P4 (Việt Nam — APJM)` → `P3 (Việt Nam — APJM)`

Grep trong 3 files: ~12 instances cần đổi.

---

### C5 — Bảng 4.9 Emerging sub-breakdown: VNM n thiếu nhất quán với §5.3

**Vị trí**: `thesis/15` §4.9 Bảng 4.9:
- "Emerging — FDI dẫn dắt SEA (VNM, IDN, PHL)": n_firms = 13,779 — đây là 3 nước gộp
- §5.3 nói Vietnam riêng n=3,077 → không thể cross-check
...
### M1 — §7.3.4 và §7.4 items 1–6 và §7.5: Placeholder text, không có nội dung

**Vị trí**: `thesis/16`
- §7.3.4: `(Giữ nguyên từ v3.1a — xem commit b41dbb8...)` — trống
- §7.4: `(Giữ nguyên từ v3.1a — 6 hạn chế.)` — items 1-6 không được viết ra
- §7.5: `(Giữ nguyên từ v3.1a — 4 giai đoạn...)` — trống

**Mức độ**: Ba sections quan trọng nhất — hạn chế nghiên cứu, hàm ý CĐ2, kế hoạch hoàn thiện — **hoàn toàn trống**.

**Hành động**: Viết đầy đủ nội dung 3 sections này. Ưu tiên §7.4 (hạn chế) vì phản biện luôn đọc section này trước.
...
**Vị trí**:
- `thesis/16` §7.2: "**CĐ2 mở rộng 9 phân nhóm con**"
- `thesis/15` §4.7(b): "**8 phân nhóm con** với hiệu ứng cố định"
- `thesis/16` §7.3.2: "**Phân loại 8 phân nhóm con cho CĐ2**"

**Hành động**: Thống nhất 1 con số và liệt kê rõ danh sách 8 hoặc 9 sub-regimes.

---

### M4 — §2.3 framing tạo impression H1-H6 unanchored đến khi P6 xong

**Vị trí**: `thesis/14` §2.3:
...
**Hành động**: Sửa §2.3: *"CĐ1 cung cấp existence evidence cho H1-H6 (Chương 4-6). P6 sẽ bổ sung magnitude evidence từ pooled effect sizes — phân biệt hai vai trò."*

---

### M5 — §4.5.6 Bảng 4.5.6 gender data: explicitly estimates, not calculated values
...
### m1 — H7: §1.5 introduce H7 nhưng §4.7 và §7.3.4 chỉ liệt kê H1-H6
...
**Hành động**: Thêm note tại §4.6 phân biệt schema effect vs xu hướng thực.
...
| Metric | §4.1 text | Bảng 4.1 | §5.3/5.8 | §4.4.5.1 | Status |
|---|---|---|---|---|---|
...
| Singapore n | — | — | 623 | 623 | ✓ consistent |

---

## CROSS-CHAPTER CONSISTENCY: §2.3 ↔ §7.4(8)

| Item | §2.3 (file 14) | §7.4(8) (file 16) | Status |
|---|---|---|---|
| P6 status | "in preparation" | "chưa hoàn thành" | ✓ consistent |
| P6 role | "empirical anchor H1-H6" | "meta-analysis" | ✓ consistent |
| CĐ1 substitute method | "systematic narrative review" | "PRISMA 2020" | ✓ consistent |
...
|---|---|---|---|---|---|
| H1 | Phi tuyến I→P | ✓ (β₂<0) | ⚠️ cần specify | Lu & Beamish (2004) | TP VN 39–46%; TQ 47–49% |
| H2 | TCI điều tiết | ✓ | ⚠️ cần specify | Cohen & Levinthal (1990) | Bảng 6.1 TCI +0.128 Advanced |
| H3 | DAI có điều kiện | ✓ | ⚠️ by regime | Verhoef 4-Tier; CDCM | Bảng 6.1 DAI -0.129 Advanced |
| H4 | Phân nhóm con | ✓ | ⚠️ 8 hay 9 sub? | Hall & Soskice VoC | Bảng 4.1 phân tán divergent |
| H5 | Cụm tài nguyên | ✓ | ⚠️ cần specify | Hertog (2010) | §5.2 sd log Vịnh 0.31–0.47 |
| H6 | Forced I penalty | ✓ | ✓ âm | Briguglio (1995) | §5.7 SIDS Kiribati FSTS 1% |
...
| Completeness | 60% | §7.3.4, §7.4(1-6), §7.5 trống |
...
2. **C2**: Fix 2025 cohort n 16,979→17,107 trong §4.1 và Bảng 4.10 (~10 min)

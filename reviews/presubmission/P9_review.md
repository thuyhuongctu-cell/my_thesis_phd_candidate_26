...
2. **Reconcile the pooled-N discrepancy.** Body/abstract report pooled analytic N = 28,717 (= 8,941+9,300+10,476), but §4.4 reports the pooled 3-wave model at "N = 28,742." Resolve and state why (e.g., extra moderator deletions) or correct the typo.
...
|---|---|---|---|---|
| M-1 | MAJOR | Intro §1 ("three contributions"); abstract Originality | Contribution/hypothesis count mismatch. Intro and prompt frame "three contributions" and the README implies H1–H3, but the paper actually tests **H1, H2a/H2b, H3a/H3b, H4a/H4b**. The digital-infrastructure (H4) result is one of the three headline contributions yet is not labelled in the H1–H3 numbering. | State explicitly that the three contributions map to H2 (threshold dissolution), H4 (public/private complement-not-substitute), and H3 (TCI sign-flip), with H1 as baseline. Make the H-numbering and contribution-numbering consistent. |
| M-2 | MINOR | §2.5 / Figure 1 caption | Conceptual model claims to integrate "H1 to H4b" but the three stated contributions in §1 do not name H4 as a contribution explicitly — chain is sound but the reader must reconstruct the mapping. | Add one sentence linking each contribution to its hypothesis number. |
| M-3 | MINOR | Abstract Findings ↔ §4.4 | Abstract reports "FSTS × wave_2025 = −1.63 (p<0.0001)"; body confirms. Consistent. (No fix — noted as verified.) | — |

Intro chain (institutional churn → untested scope condition → 3-wave test) is clear and the conclusions are supported by the results. H1–H4 are all tested and reported. Abstract is structured (Purpose/Design/Findings/etc.) — acceptable for MIR even though Emerald-style; flagged only for length.
...
| W-2 | MINOR | §4.4 and §5.1 | The linear-trend and wave-dummy results are restated almost verbatim in Results §4.4 and Discussion §5.1 (FSTS×wave_2025 = −1.63; FSTS×trend = −0.10). Mild redundancy. | Trim the §5.1 restatement to a back-reference. |
...
| ID | Sev | Location | Issue | Fix |
|---|---|---|---|---|
| G-1 | MINOR | §5.2 line 351 heading | "a complementarity, not substitution" — parallelism off. | "a complementarity, not a substitution" or "complements, not substitutes." |
...
| N-5 | MINOR | Prior-work disclosure | Do & Phan (2025) IntechOpen disclosure is present in abstract, §1, §6, acknowledgements, cover letter, and title page, in third person ("the authors' earlier work"). Compliant. | No fix — verified. |

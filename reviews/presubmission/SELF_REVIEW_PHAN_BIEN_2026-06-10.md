...
Cùng file còn 2 lỗi:
- **Dòng 25:** *"P5, under review at IJOEM"* / *"P3, under review at JED"* — **sai sự thật** cho đến khi các bài đó thực sự được nộp. Nếu P9 nộp trước, đây là tuyên bố trạng thái giả đến rủi ro liêm chính với editor. Phải sửa thành "manuscript in preparation/under submission preparation".
...
`p6/osf/P6_OSF_Preregistration_Template.md` Mục 5 vẫn mô tả chiến lược **census WoS+Scopus đầy đủ** kèm *"validated against a 30-paper known-item set (recall 29/30 = 97%)"* và Mục 6 "two screeners + third reviewer" — trong khi cả 4 manuscript đã chuyển sang **citation-anchored bounded search**. Checklist đang hướng dẫn NCS đăng ký template này: nếu đăng ký nguyên trạng, **OSF registration và manuscript mâu thuẫn nhau công khai, có timestamp** — tệ hơn không đăng ký. Template **phải được cập nhật trước khi đăng ký OSF**. Tương tự, `thesis/p6_prisma_protocol.md:177` ("2 reviewers độc lập sàng lọc") trong luận án cũng cần đồng bộ.
...
|---|---|
| `p9_india_vi.md` | chứa **cả** 28.742 (cũ) **và** 28.717 (mới) đến tự mâu thuẫn nội bộ; abstract bản dài chưa rút gọn; danh mục TLTK trước khi gỡ 13 orphan |
...
`p8/.../01_manuscript_blinded.md:282`. Caveat chỉ mới được thêm ở abstract + lần đầu trong Mục 4. Một reviewer đọc Conclusion độc lập sẽ hiểu sai cơ sở ước lượng. Cần lặp ngắn gọn caveat ở Conclusion (cả 3 package).

### B3. P8 — Bảng 1 ghi **"N (firms)"** nhưng đếm **firm-year observations**
Fiji 165 trải 2 đợt 2009+2025 (không có panel linkage) đến 165 là quan sát doanh nghiệp-năm, không chắc là doanh nghiệp duy nhất. Kế thừa từ wording gốc ("959 firms across multiple country-year waves") nhưng nên ghi "Obs." cho chính xác.
...
**Thứ tự sửa đề xuất:** A1 (P9 MIR cover) đến A2+A3 (P6 ICR tense + third reviewer, 4 package) đến A4 (OSF template + protocol luận án) đến B1 (đồng bộ VI) đến B2–B5.
...
## F. TRẠNG THÁI XỬ LÝ (cập nhật 2026-06-10, đã kiểm chứng)

| Mục | Vấn đề | Trạng thái |
|---|---|---|
| **A1** | P9 cover kể chuyện durability ngược kết quả + "under review" sai + 22,000 obs | Có **SỬA** — "dissolves rather than relocates"; "in preparation"; 28,717 |
| **A2** | P6 ICR thì quá khứ cho việc chưa làm | Có **SỬA** — chuyển present-tense protocol (4 package) |
| **A3** | P6 "third reviewer / two screeners" không tồn tại | Có **SỬA** — "two authors… resolved by discussion" (4 package + OSF) |
| **A4** | OSF template còn census + third reviewer | Có **SỬA** — reframe citation-anchored + 2 tác giả |
| **B1** | Bản dịch VI lệch pha (P9 28.742, P5 217-panel, P6 90/78) | Có **SỬA** — đồng bộ với EN |
| **B2** | P8 Conclusion thiếu caveat N=209 | Có **SỬA** — thêm estimation N=209 (3 package) |
| **B3** | P8 Bảng 1 "N (firms)" vs firm-year obs | Có **SỬA** — ghi rõ "firm-year observations" |
| **B4** | P9 MIR abstract structured (sai chuẩn Springer) | Có **SỬA** — chuyển unstructured 236 từ (IJOEM/JABS giữ 7-part) |
| **B5** | P4 N=84 chỉ làm rõ ở mbr | Có **SỬA** — thêm vào mir + apbr |
| C1–C4 | MINOR (keywords, tên tạp chí blinded, đổi từ AI-tone, _source filename) | ⏳ tùy chọn — chưa xử lý (thấp ưu tiên) |

**Tất cả 4 CRITICAL + 5 MAJOR đã được sửa và kiểm chứng bằng grep.** Còn lại chỉ MINOR (C1–C4) mang tính tùy chọn.

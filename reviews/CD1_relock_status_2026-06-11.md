...
3. **Chạy thử trên raw có sẵn trong repo** (5 nền trong khung: Azerbaijan, Cambodia, Korea, Laos, Taiwan) đến pipeline ra số thật.

## Hai nút thắt còn lại cho re-lock TRUNG THỰC

### (A) Dữ liệu chưa lên container
- Drive-MCP chỉ trả base64 **qua context của tôi** đến ~120MB nhị phân không kéo bulk được; container github-only nên không `wget` Drive.
- **Cần:** push 44 nền còn lại vào `data_wbes/raw_dta/` (git) HOẶC attach vào chat ( đến `/root/.claude/uploads`, pipeline đã đọc). Một lần, không tốn context, đầy đủ.

### (B) Hiệu chỉnh phương pháp (calibration) — ĐÃ GIẢI QUYẾT
- Nguyên nhân sd demo ban đầu cao (2,2–4,4): tính sd **gộp** xuyên nước đến bị thổi bởi chênh mức giữa nước.
...
- **Kết quả kiểm chứng trên raw có sẵn (Korea/Taiwan vs Azerbaijan/Cambodia/Laos):**
 ...
 đến Pipeline **tái lập đúng gradient phân tán của CĐ1**, không bịa. Phương pháp đã khóa.
...
3. Chạy toàn bộ đến tạo bảng mô tả CĐ1 tái lập được.
4. Thay các bảng CĐ1 (2.3.2.1, 2.3.4.1, phụ lục A.1…) bằng số tái lập, cập nhật crosswalk đến re-lock hoàn tất; regenerate docx.

...
| R&D%, ISO%, đổi mới sản phẩm | **KHÔNG có** | có (Bảng 2.3.4.1) | File chỉ có TCI tổng hợp |
 ...
 đến **File phân tích P7 ≠ pipeline mô tả CĐ1.** Tái tính bảng mô tả CĐ1 (FSTS/website/FDI/R&D/ISO/sd theo nhóm) từ file này sẽ ra **số sai** (sd 3,43 thay vì 1,03; FDI 0%). **Cấm dán.**

**Dữ liệu thô (raw WBES) cho đủ 49 nước KHÔNG có trong repo** (`raw_dta/` chỉ 9 nền + India). đến Không thể chạy lại pipeline CĐ1 trong môi trường hiện tại.
...
| # | Việc | Nguồn chuẩn |
|---|---|---|
| B1 | Đếm nền kinh tế: **47 đến 49** (Á–TBD) | dữ liệu (52 − Comoros/Turkey/Cyprus) |
| B2 | Pool phân loại: **101.185 đến 96.415**; mẫu hồi quy **91.864** | dữ liệu |
...
> ⚠️ **Cảnh báo cascade:** B1–B5 đổi membership đến **các bảng % mô tả theo nhóm trong CĐ1 phải tái tính lại** (vì Sri Lanka/Jordan/Bangladesh/Pakistan chuyển nhóm làm đổi mọi trung bình nhóm). Do đó B1–B5 **chỉ trọn vẹn** khi đi kèm phần C (tái tính), nếu không lại tạo mâu thuẫn mới.
...
- Sri Lanka, Jordan đến nhóm **Emerging (V)** theo dữ liệu, KHÔNG phải Nhóm IV như CĐ viết. *(Kiểm tra: GNI/capita Sri Lanka ~$3.8k, Jordan ~$4.4k — gần ngưỡng IV/V; NCS xác nhận.)*
- Bangladesh, Pakistan đến **Lower_mid (IV)** theo dữ liệu, KHÔNG phải Frontier như CĐ. *(GNI Bangladesh ~$2.8k, Pakistan ~$1.5k — biên IV/V; xác nhận.)*
- Timor-Leste đến **SIDS (VI)** theo dữ liệu, KHÔNG phải Frontier.
- Oman đến **Advanced_resource (II)** — CĐ hiện thiếu Oman.
...
**Bước 2 — cần raw WBES 49 nước + script CĐ1 (NCS cung cấp hoặc phiên có raw):** chạy lại pipeline mô tả CĐ1 (winsorize 1/99 trong cụm, R&D/ISO/đổi mới, FDI) trên khung 49 nước đến thay toàn bộ bảng % mô tả + n_firms; đổi 101.185 đến 96.415, 47 đến 49. Regenerate docx.
...
- **Tuyệt đối không** dán số từ `p7_pooled_clean.csv` vào bảng mô tả CĐ1 (sai pipeline đến sai số).
...
| `data_wbes/analysis/pooled_wbes_6waves.csv` | chỉ **3 nước** (CHN/SGP/VNM) | Có | Không (sd~3,82) |
| `data_wbes/p7/p7_pooled_rich.csv` | 52 nước | Không (chỉ TCI/DAI tổng hợp) | Không |
| `data_wbes/p7/p7_pooled_clean.csv` | 52 nước | Không | Không (sd 3,43) |

 đến **Raw WBES đủ 49 nước + đúng pipeline mô tả CĐ1 không có trên bất kỳ nhánh nào.** Bước 2 (re-lock số mô tả) **không thể thực thi** trong môi trường hiện tại mà không bịa.

**Cần NCS:** upload trực tiếp **raw WBES đủ 49 nền** (hoặc file pool 101.185 gốc CĐ1 với đủ biến website/R&D/ISO/đổi mới/FDI + LP đã winsorize) đến khi đó máy chạy re-lock đầy đủ + đồng bộ docx.
...
- **Raw `.dta` CHỨA đủ biến CĐ1 cần**: `h8`(R&D), `b8`(ISO), `h1`(đổi mới SP), `h5`(quy trình), `c22b`(website), `e6`(công nghệ NN), `d3c`(FSTS), `d2/l1`(LP), `b2b`(FDI). đến Pipeline tái lập được NẾU có đủ raw.
...
Afghanistan, Armenia, Bahrain, Bangladesh, Bhutan, Brunei, China, Fiji, Georgia, HongKong, Indonesia, Iraq, Israel, Jordan, Kazakhstan, Kiribati, Kuwait, KyrgyzRepublic, Lebanon, Malaysia, Maldives, Mongolia, Myanmar, Nepal, Oman, Pakistan, PapuaNewGuinea, Philippines, Qatar, Samoa, SaudiArabia, **Singapore**, SolomonIslands, SriLanka, Tajikistan, Thailand, TimorLeste, Tonga, Turkmenistan, Uzbekistan, Vanuatu, **Vietnam**, Yemen.
 ...
 đến **Để re-lock CĐ1 đầy đủ:** commit/upload 43 raw `.dta` này vào `data_wbes/raw_dta/`. Khi đó tôi: mở rộng build script giữ R&D/ISO/đổi mới đến dựng pool rich 49 nước đến re-lock toàn bộ CĐ1+CĐ2+thesis một pass nhất quán đến regenerate docx đến chạy MASTER kiểm tra.
...
- Bảng 2.8 + Bảng 2.11 + Mục 2.1.1: n_firms canonical (I 4.222·II 2.269·III 13.993·IV 50.926·V 18.569·VI 1.885 = 91.864) + membership theo nhãn dữ liệu (Oman đến II; Bangladesh/Pakistan đến IV; Sri Lanka/Jordan đến V; Timor-Leste đến VI) + crosswalk tên-CĐ↔icrv_label.
- 101.185 đến 91.982, 47 đến 49 toàn bộ (VN + EN abstract); 5+5+6+7+17+7 đến 5+6+6+7+17+8.

...
Referee-lens audit of variable measurement (Chương 3 Section 3.3) and methodology
(Section 3.4–3.5), in response to the question of whether the variable descriptions and
methods are solid.
...
- Identification (Section 3.5.6): two-way FE + IV (first-stage F, over-id) + Oster (2019)
...
## Fixed in this pass
1. **TCI item-code error (P5 China, Section 3.4.5.3 + its variable table):** TCI_full was
...
2. **Section 3.3.4** now states the TCI thin/full operationalization explicitly and notes
...
3. **Section 3.5.6** measurement-invariance limitation across the 3 WBES questionnaire
...
against. Recommendation: re-run build to estimate from the definitive raw set, refresh the
...
2. **Estimator control bug** (`02_run_p7_models.py`): the default control set included
   ...
   (≈79k to ≈32k) and made the FSTS curvature look "not confirmed". Removed from defaults.
3. **Reconciliation result (incl. Japan, thesis-consistent controls):** the fully-specified
   ...
   reported range. Thus, the thesis's substantive P7 claims (inverted-U + TCI-positive) **hold
...
- `01_build_p7_dataset.py` (raw WBES .dta to `p7_pooled_clean.csv`, 50 economies)
...
- leads to Thesis Section 3.4.5.4 (P7: b8 + e6, 2-item thin) is **correct**; no estimate change.
  Section 3.3.4 reconciliation note updated from "needs checking" to "confirmed".
...
- Repeated cross-sections to firm-level causal inference is bounded (handled via

...
| 7 | CIMT (Capability-Institution Mismatch Theory) absent | 🟡 MEDIUM | 1-2h | ✅ **RESOLVED 2026-06-03 (revised; CIMT removed per NCS decision)**: NCS reviewed the CIMT framing and confirmed it is not an established IB theory but a novel author proposition. Per scholar-review analysis (`thesis/CIMT_REALITY_AND_TOP_1PCT_SCHOLAR_REVIEW.md`) flagging the "theory inflation" risk under Whetten (1989) criteria, CIMT name was **removed** from P6 + P7 manuscripts. §2.2 (P6) and §2.1 (P7) now frame the underlying 3 channels — rent protection (Kogut & Zander 1993, Zaheer 1995), LoF attenuation (Peng et al. 2008, Zaheer 1995), and institutional-void amplification (Khanna & Palepu 2010) — as an *integrated capability-institution mechanism* synthesised from established literature, without coining a new named theory. P7 §5.1 reverted to 4-contribution structure (relocated optimum + digital-for-institutional substitution + level-relocation distinction + ICRV framework). Abstract reframed away from "Capability-Institution Mismatch mechanism" name. This is the safer scholarly path that avoids reviewer pushback at JIBS / APJM and removes the cross-paper inconsistency (CĐ2/Ch.2 luận án never used CIMT). |
| 10 | Country-clustered SE robustness column missing | 🟡 MEDIUM | 1h | ✅ **RESOLVED 2026-06-03**: New §4.9 "Country-Year Cluster-Robust Standard Errors" panel added. Compared HC1 vs CR1 (country-year clustering, G = 112 for M2 sample; G = 62 after listwise deletion on controls). Cluster SE is 2-3× wider than HC1 but ALL focal H1 inverted-U coefficients remain statistically distinguishable: M2 (p < .001), M2_ctrl (p < .001), M5+TCI (p < .01), M8+DAI (p < .05), M9+managerial (p < .05). No inferential change. Stata replication: `p7/replication/do/03_p7_cluster_se.do`. |
| 12 | Abstract conversion to structured Purpose/Design/Findings/Originality | 🟡 LOW | 30min | ✅ **RESOLVED 2026-06-03**: Restructured to JIBS 7-section format (Purpose / Design / Findings / Research limitations / Practical implications / Social implications / Originality), 349w total. CIMT framing leads as Purpose; all canonical numerics preserved (N = 82,302→28,500; TP ≈ 37%; DAI×ICRV β = +0.052, p = .049; cross-paper SIDS FIP cross-reference). |

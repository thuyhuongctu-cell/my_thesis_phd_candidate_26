...
\title{P8\_PacificSIDS\_WorldDev\_EN}
...
Using World Bank Enterprise Survey data from 1,469 firms across nine
...
Specifically, \(\beta = −0.404\), p = .032 in the country-and-year
fixed-effects specification, and \(\beta = −1.236\), p \textless{} .001
...
This pattern is consistent with the FIP mechanism: when structural
prerequisites are absent, firm-level capabilities cannot redirect the
I-P relationship toward positive territory. The findings reframe the
inverted-U paradigm as conditional rather than universal, identify
boundary conditions for institutional moderation theory, and carry
direct implications for the Antigua and Barbuda Agenda for SIDS.
...
in 75 years. Some 54 economies remain more than a century from reaching
developed-country standards at current trajectories, and SIDS are
...
initially dominate. Beyond a threshold, however, mounting organizational
...
.052). Foreign ownership shows a marginally negative association
(\(\beta = −0.006\), p = .056), perhaps reflecting thin foreign
investment efficiency in SIDS contexts; firm size is positive but
non-significant (β = +0.082, p = .242).
...
\item
  ...
  (\emph{p} = .265), \textbf{not significant and negative in sign}; no
...
M3 (\(\beta = −0.974\), p = .252), likely reflecting the substantially
reduced sample (N = 526 for M3, versus N = 1,469 for M1, due to missing
...
confirmed across four points. (i) a statistically significant negative
linear FSTS--performance relationship (M1: \(\beta = −0.404\), p =
.032). (ii) no significant quadratic curvature or turning point (M2:
both FSTS\_c and FSTS\_c² non-significant); (iii) robustly negative
across four specifications (M\_yearFE: \(\beta = −1.236\), p \textless{}
.001; M\_bivariate: \(\beta = −1.596\), p \textless{} .001;
exporters-only: \(\beta = −0.901\), p = .027); and (iv) null capability
moderation (M3: TCI\_z p = .495; DAI\_z p = .402).
...
support for FIP across four empirical specifications:
\(\beta = −0.404\), p = .032 in the primary country-and-year
fixed-effects model; \(\beta = −1.236\), p \textless{} .001 without
country fixed effects; and \(\beta = −0.901\), p = .027 in the
...
result (\(\beta = −0.901\)) addresses adverse selection concern: among
active exporters, intensifying exports further worsens performance.}

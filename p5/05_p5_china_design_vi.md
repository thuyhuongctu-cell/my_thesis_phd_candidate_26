...
> **Cập nhật trạng thái (2026-05-30):** Phân tích empirical đã hoàn thành và được báo cáo trong luận án Chương 4 Mục 4.4.3 (mẫu N = 2,610 năm 2012; N = 1,934 năm 2024; tổng gộp N = 4,544). Tổng hợp kết quả chính: chữ U ngược xác nhận trong cả hai wave (β₁ = +1.28/+1.19 dương có ý nghĩa; β₂ = -1.53/-1.58 âm có ý nghĩa); điểm uốn ổn định cấu trúc (TP = 49.4%/47.2%/48.8% gộp; Paternoster z(FSTS) = +0.82, p = .412; z(FSTS²) = -0.61, p = .545); TCI nâng mặt bằng (β₃ = +0.28/+0.43, p < .001) nhưng không điều tiết độ cong; **tương tác ba chiều FSTS × Year (H5-P5) không sống sót Bonferroni** (F1 = 3.26, p = .039 nhưng α* = 0.017 với 3 kiểm định). Khi xây dựng manuscript đầy đủ, kết quả H5-P5 phải được trình bày minh bạch là *không xác nhận* sau hiệu chỉnh đa kiểm định, để tránh báo cáo chọn lọc.
...
> **Lưu ý đánh số:** Các ký hiệu H1-P5, H2-P5, ... là đánh số nội bộ của bài báo P5, không trùng với hệ giả thuyết H1-H6 của luận án tổng thể (xem Chương 2 luận án). Khi viết manuscript, nên đổi tên thành H1, H2, ... và làm rõ phạm vi nội bộ.
...
> *Kết quả thực tế (Chương 4 §4.4.3):* H5-P5 **không xác nhận** sau hiệu chỉnh đa kiểm định Bonferroni. Tương tác $FSTS² \times Year_{2024}$ trên TCI cho F1 = 3.26, p = .039 — đạt α = .05 thông thường nhưng không vượt ngưỡng Bonferroni α* = 0.017 (3 kiểm định họ). Khi viết manuscript, nên (i) báo cáo cả F-statistic gốc và quyết định không-xác-nhận sau Bonferroni; (ii) tránh ngôn ngữ kiểu "marginal significance" cho p = .039 vì đã thất bại ngưỡng tiền đăng ký; (iii) trình bày như null finding informational consistent với hằng số sinh tồn quan hệ I-P qua một thập kỷ chuyển đổi số Trung Quốc.
...
| H5-P5 không sống sót Bonferroni | Reframe trong manuscript thành phát hiện "structurally stable I-P relationship" (positive theoretical signal) thay vì spin marginal F = 3.26 thành significant; chỉ rõ pre-registered α* = 0.017 đã không đạt |
| 217 firms repeat between 2012 và 2024 waves (overlapping cohort) | Standard errors cluster theo `idstd` đã áp dụng để xử lý correlation trong nhóm lõi; báo cáo minh bạch trong methodology |
| Three-way interactions có khả năng underpowered | Tính power ex-ante cho F-tests theo Leon (2004) trước khi commit interpretation; báo cáo |

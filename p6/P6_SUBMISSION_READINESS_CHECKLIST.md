...
> Optional upgrade (author): if a full formal WoS/Scopus census is run later, the flow can be re-expressed with stage-level counts (tooling: `p6/tools/28_extract_wos_scopus_locally.py`; search strings in `p6/osf/P6_OSF_Preregistration_Template.md` Section 6). This is **no longer a submission blocker**.
...
- [ ] Compute Cohen's κ (categorical) and ICC(2,1) (cDAI) using the R snippet in `p6/tools/p6_extraction_codebook.md` Section 7.
...
- [ ] Replace the `[insert OSF DOI at submission]` placeholder (already in all four packages) with the **actual OSF DOI/URL** (Methods Section 3 and the data-availability statement).
...
- [x] "extracted manually" to "extracted" in Methods (the false absolute removed).
...
Per the audit (B2): publication bias halves the pooled effect (r 0.074 to 0.035), and two of three novel moderators are null (the ICRV result is driven by a k = 3 Frontier cell). Before submission:

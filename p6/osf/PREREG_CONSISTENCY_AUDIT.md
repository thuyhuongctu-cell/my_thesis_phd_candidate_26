...
- **Moderator-table CSVs (`QM` column): STALE.** ⚠️ Code fixed (see §4b); **one R re-run of
  `p6_real_mara.R` still required** to regenerate `table2/3/4` before the package goes public.
- **Prereg record:** unchanged (frozen). One open pre-public step remains (CSV regeneration).
...
**Fix applied to code:** the script now also fits the prereg-specified with-intercept model
(`~ icrv_f`, `~ cdai_f`, `~ dpl_f` → `m1b/m2b/m3b`) and writes *that* between-group Q_M/p to the
moderator tables; console labels corrected (cDAI = H3, DPL = H2). **The CSVs were NOT hand-edited**
(no R in this environment — transcribing test statistics would risk fabrication). The
moderator-table CSVs must be **regenerated by running the corrected `p6_real_mara.R`** (R + metafor)
before the package is made public; until then the manuscript values (17.35 / 1.23 / 0.56) are
authoritative and the CSV `QM` columns are known-stale.

...
class WOSVerifierMutexDOI:
    ...
    def __handle_subchunks(self, doi_chunk: list, id_chunk: list):
        self.chunk_part += 1

        print("Chunk part: ", self.chunk_part, "is being processed. Size: ", len(doi_chunk))
        
        with ProcessPoolExecutor(max_workers=MAX_THREAD_POOL) as executor:
            futures = [executor.submit(self.__verify_chunk_with_handles, chunk, id_chunk[i]) for i, chunk in enumerate(doi_chunk)]
        ...
        for i, chunk in enumerate(doi_chunk):
            thread = threading.Thread(target=self.__verify_chunk_with_handles, args=(chunk, id_chunk[i]))
            thread.start()
        ...
        # evidently it will leave trash in the temp folder
        self.__firefox_cleanups()
            
        print("Chunk part: ", self.chunk_part, "has been processed.")
        
    # HACK: To avoid the problem of the thread abuse and RAM being at 99%, we will split by chunks again.
    def __divide_chunks(self, doi_chunks : list, id_chunks : list, max_subchunks = 2):
        chunk_size_counter = 0
    
        for doi_chunk, id_chunk in zip(doi_chunks, id_chunks):
            doi_subchunks = [doi_chunk[i:i + len(doi_chunk) // max_subchunks] for i in range(0, len(doi_chunk), len(doi_chunk) // max_subchunks)]
            id_subchunks = [id_chunk[i:i + len(id_chunk) // max_subchunks] for i in range(0, len(id_chunk), len(id_chunk) // max_subchunks)]

            self.__handle_subchunks(doi_subchunks, id_subchunks)
                
        print("Total counter: ", chunk_size_counter)
    ...
    def verify_doi(self, data: pd.DataFrame):
        ...
        # fix the memory problem and rust_mozprofile filling up the temp folder
        print("[WOSMutex] Dividing chunks into: ", MAX_SUBCHUNKS, "subchunks.")

        self.__divide_chunks(doi_chunks, id_chunks, MAX_SUBCHUNKS)
    ...
    def __verify_chunk_with_handles(self, doi_chunk: list, id_chunk: list):
        ...
        for id in windows_data:
            ...
            try:
                ...
                print("[BIG-WARNING] More than one result found for a unique DOI. Skipping...", "DOI:", doi, "Results: ", result_count)
                # write to a debug_dupes.log file
                with open(os.path.join(DATA_PATH, "debug_dupes.log"), "a") as f:
                    f.write(f"DOI: {doi} ID: {id} Results: {result_count}\n")

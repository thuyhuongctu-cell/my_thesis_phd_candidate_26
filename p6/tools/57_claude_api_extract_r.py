#!/usr/bin/env python3
"""
57_claude_api_extract_r.py — Claude API batch r-extraction from PDFs
...
and no converted_r yet:
  ...
  3. Asks Claude API to identify the I→P effect size and convert to r
  4. Writes result back to fulltext_to_extraction_tracker_v3.csv

Usage:
    python3 57_claude_api_extract_r.py \
        --queue   p6/tools/results/extraction_queue_20260520.csv \
        --tracker p6/tools/results/fulltext_to_extraction_tracker_v3.csv \
        --pdf-dir p6/pdfs \
        --log     p6/tools/results/claude_extract_log_20260523_1200.csv \
        --limit   50 \
        [--manifest p6/tools/results/oa_manifest_20260520.csv] \
        [--dry-run]

Requires:  pip install anthropic pdfplumber requests
Secret:    ANTHROPIC_API_KEY env variable
"""

import csv, math, os, re, sys, argparse, time
...
EXTRACTION_PROMPT = """\
You are a meta-analysis research assistant. Your job is to extract the main \
...
def ask_claude(client: anthropic.Anthropic, text: str) -> dict:
    """Send excerpt to Claude and parse JSON response."""
    import json
    ...
    try:
        ...
        return {"converted_r": None, "conversion_formula": "error",
                "sample_size_n": None, "notes": f"Claude error: {e}"}
...
def main():
        ...
        if tracker_row and not args.dry_run:
            if not tracker_row.get("converted_r", "").strip():
                ...
                if notes_key in tracker_row:
                    existing = tracker_row.get(notes_key, "")
                    tracker_row[notes_key] = (
                        existing + " | CLAUDE: " + log["notes"]
                    ).strip(" |")
    ...
    print(f"\nNEXT: Review CLAUDE: entries in notes_for_extractor, then set ready_for_r=1")

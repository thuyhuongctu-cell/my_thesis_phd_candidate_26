...
|---|---|:-:|---:|---|
| 6 | **3-journal target confusion (CRITICAL)** | 🔴 CRITICAL | 2h | ✅ **RESOLVED 2026-06-03**: NCS confirmed target = **Asia Pacific Journal of Management (APJM, Springer Nature, Q1, ABS-3)**. Rationale: (i) perfect geographic fit for the 45-economy Asia-Pacific corpus and the ICRV taxonomy; (ii) Q1 status within the user-specified Q1–Q2 range (MRQ Q3–Q4 excluded; IBR excluded by user; MIR harder + slower review; APJM is the optimum); (iii) APJM editorial board includes institutional-theory experts (Peng 2003 IBV anchor) welcoming the Capability-Institution Mismatch theoretical contribution; (iv) 6–8 month review timeline vs MIR 8–12 months; (v) Springer EM portal NCS already familiar. **Implementation:** manuscript header line 5 + footer line 511–512 updated to APJM; new `p6/submission/apjm_package/` created with title page + APJM-specific cover letter + README; OSF DOI z37kn unchanged (preregistration body is journal-agnostic — no retro-edit). Build script updated to emit APJM package. Submit with current k = 238 (Path A); Path B WoS/Scopus expansion to k ≈ 600–700 pre-registered as follow-up replication. EN cover letter only (per NCS choice). |

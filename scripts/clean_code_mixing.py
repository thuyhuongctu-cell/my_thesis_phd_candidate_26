...
RULES = [
    ...
    # bổ sung đợt 4 (rà code-mixing toàn 5 chương)
    # -- cụm dài + biến thể viết hoa đầu câu (đặt trước từ đơn) --
    (r"\bmixed synthesis[- ]empirical design\b", "thiết kế hỗn hợp tổng hợp–thực nghiệm"),
    (r"\bMixed design\b", "Thiết kế hỗn hợp"),
    (r"\bmixed design\b", "thiết kế hỗn hợp"),
    (r"\bTwo[- ]way interactions?\b", "Tương tác hai chiều"),
    (r"\btwo[- ]way interactions?\b", "tương tác hai chiều"),
    (r"\bThree[- ]way interactions?\b", "Tương tác ba chiều"),
    (r"\bthree[- ]way interactions?\b", "tương tác ba chiều"),
    (r"resource[- ]orchestration logic", "logic điều phối nguồn lực"),
    (r"measurement harmonization protocol", "giao thức hài hòa hóa đo lường"),
    (r"cấu trúc purity", "độ thuần cấu trúc"),
    (r"\bprimary data\b", "dữ liệu sơ cấp"),
    (r"\binclusion criteria\b", "tiêu chí lựa chọn"),
    (r"\bdigital transformation\b", "chuyển đổi số"),
    (r"\bdigitali[sz]ation\b", "số hóa"),
    (r"\btechnological capability\b", "năng lực công nghệ"),
    (r"\bdigital capability\b", "năng lực số"),
    (r"\bdigital adoption\b", "chấp nhận số"),
    (r"\beffect sizes\b", "cỡ ảnh hưởng"),
    (r"\beffect size\b", "cỡ ảnh hưởng"),
    (r"\brandom[- ]effects?\b", "hiệu ứng ngẫu nhiên"),
    (r"\bfixed[- ]effects?\b", "hiệu ứng cố định"),
    (r"\bturning point\b", "điểm uốn"),
    (r"\bpanel data\b", "dữ liệu bảng"),
    (r"\bsector FE\b", "ngành (FE)"),
    (r"\bcoverage time\b", "phạm vi thời gian"),
    # -- từ đơn: biến thể hoa trước, thường (IGNORECASE) sau --
    (r"\bSubgroups?\b", "Nhóm con"), (r"\bsubgroups?\b", "nhóm con"),
    (r"\bPhase\b", "Giai đoạn"), (r"\bphase\b", "giai đoạn"),
    (r"\bMeta\b", "Phân tích tổng hợp"),
    (r"\bEmpirical\b", "Thực nghiệm"), (r"\bempirical\b", "thực nghiệm"),
    (r"\bInteractions?\b", "Tương tác"), (r"\binteractions?\b", "tương tác"),
    (r"\bOutcomes?\b", "Kết quả"), (r"\boutcomes?\b", "kết quả"),
    (r"\bCapabilit(y|ies)\b", "Năng lực"), (r"\bcapabilit(y|ies)\b", "năng lực"),
    (r"\bThree stages\b", "Ba giai đoạn"), (r"\bthree stages\b", "ba giai đoạn"),
    (r"\bBaseline\b", "Cơ sở"), (r"\bbaseline\b", "cơ sở"),
    (r"cross[- ]sections?", "lát cắt ngang"),
    (r"\bcomposite\b", "chỉ số hợp thành"),
    (r"\bspillovers?\b", "lan tỏa"),
    (r"\bschema\b", "lược đồ"),
    (r"\bcoverage\b", "phạm vi"),
    (r"\bsynthesis\b", "tổng hợp"),
    (r"\bpipeline\b", "quy trình"),
    (r"\bmeta\b", "phân tích tổng hợp"),
    (r"\bsector\b", "ngành"),
]

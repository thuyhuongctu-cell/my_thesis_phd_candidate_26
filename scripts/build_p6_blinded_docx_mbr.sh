#!/usr/bin/env bash
# build_p6_blinded_docx_mbr.sh
#
# Build the blinded P6 manuscript .docx for Multinational Business Review (MBR)
...
#
# Output: p6/submission/mbr_package/01_manuscript_blinded.docx
# Prereqs: pandoc >= 3.0, python3, templates/ctu_paper_reference.docx
#
# Usage: bash scripts/build_p6_blinded_docx_mbr.sh

set -euo pipefail
cd "$(dirname "$0")/.."

SRC="p6/p6_meta_manuscript_en.md"
OUT="p6/submission/mbr_package/01_manuscript_blinded.docx"
TEMPLATE="templates/ctu_paper_reference.docx"
TMP="$(mktemp /tmp/p6_blinded_mbr_XXXXXX.md)"
trap 'rm -f "$TMP"' EXIT
...
echo "[docx] P6 MBR (blinded) -> $OUT"
pandoc \
  --from "gfm+tex_math_dollars" \
  --to docx \
  --reference-doc "$TEMPLATE" \
  --resource-path "p6:p6/submission/mbr_package:." \
...
for part in 02_title_page 03_cover_letter; do
  PART_SRC="p6/submission/mbr_package/${part}.md"
  PART_OUT="p6/submission/mbr_package/${part}.docx"
  if [[ -f "$PART_SRC" ]]; then

...
> không thể cài trong môi trường này; đã kiểm chứng `which stata stata-mp xstata` đến trống).
...
| N4 | Đồng bộ định lượng các bảng CĐ1 còn ở bản khóa (2.3.6.x, 2.3.8.x) | `CD1_PIPELINE_RESULTS.md` Mục D |
...
**Cài đặt (~2 phút)**:
1. VS Code đến Extensions đến tìm **"Stata MCP"** đến Install (extension tự khởi động server cổng 4000).
2. Đăng ký với Claude Code (1 lệnh):
   ```bash
   claude mcp add --transport http stata-mcp http://localhost:4000/mcp-streamable --scope user
   ```
3. Mở Claude Code tại repo này đến Claude gọi được `stata_run_file` trực tiếp.
...
> Mọi thứ đã chuẩn bị sẵn để chạy **trên máy NCS** — xem Mục 4.

## 3. Do-file ĐÃ VIẾT SẴN (sẵn sàng chạy ngay khi có Stata)

| File | Làm gì | Benchmark phải đối chiếu |
|---|---|---|
| `scripts/stata/00_prep_data.do` | CSV master đến `p7_analytic.dta` (khung 49 nền, **lp_z within country-year** — sửa artifact tiền tệ, fsts/fsts2, encode FE) | N ≈ 75.607 |
| `scripts/stata/p7_reestimate.do` | M2/M5 (`reghdfe` + country/year FE, cluster country) + `utest` Lind–Mehlum + TP từng nhóm ICRV + M11 moderation | **TP M5 = 40,0%**; gradient I≈28% đến V/VI≈55% |
...
- [ ] 4. Chạy `00_prep_data.do` đến kiểm tra N.
- [ ] 5. Chạy `p7_reestimate.do` đến so TP_M5 với 40,0%:
      - **Khớp (±1 điểm %)** đến spec đúng đến được phép chạy thêm Japan/sóng mới (N3) và điền bảng hệ số (N2).
      - **Lệch** đến tìm do-file gốc P7, đối chiếu PPP/weights — KHÔNG thay số khóa.
- [ ] 6. Chạy `p8_sids_fip.do` đến so β với −1,339.
- [ ] 7. Khi khớp: đối chiếu hệ số từ `dist/stata_out/*.csv` với các bảng trong bản thảo
      ...
      và cập nhật footnote Japan ở Ch4 Mục 4.6.1 thành kết quả định lượng.

...
The analytic dataset combines two waves of the World Bank Enterprise Survey for China: 2012 (full release, 2,700 firms; World Bank, 2013) and 2024 (2,189 firms; World Bank, 2025). After treatment of WBES non‑response codes -9 and -7 as missing (and the additional 2024 refusal code -8) on the focal set (sales, employees, export intensity), the post‑recoding raw frame contains 2,619 firms in 2012, 1,940 firms in 2024, and 4,559 firm‑year observations in the pooled sample. The M2 main analytic sample (sample_base), after additional listwise deletion of control variables (`lnEmp`, firm age, foreign‑ownership indicator), is 2,610 firms in 2012, 1,934 firms in 2024, and 4,544 firm‑year observations in the pooled sample. This sample_base is the basis of all wave‑specific descriptives and main threshold estimates in §4.1–§4.3.
...
**Replication note — three nested analytic samples.** Three nested samples are used consistently throughout the paper:

(i) **Post‑recoding raw frame** — 2012 N = 2,619; 2024 N = 1,940; pooled N = 4,559. Constructed from the full WBES private‑firm frame for each wave, with World Bank nonresponse codes (−9 and −7 in 2012; −9, −8, and −7 in 2024) recoded as missing on focal variables.

(ii) **Sample_base** — 2012 N = 2,610; 2024 N = 1,934; pooled N = 4,544. Obtained by additional listwise deletion across the focal control variables (`lnLP`, `FSTS`, `FSTS²`, `lnEmp`, firm age, foreign‑ownership indicator). This is the sample used for the M2 main threshold model (Table 2, §4.2), the wave-specific descriptive statistics (Table 1, §4.1), and the manufacturing-only robustness check (§4.6).

(iii) **Sample_full** — N = 3,559. Obtained by further listwise deletion on the moderator composites `TCI_full` (≥ 3 of 4 items non-missing) and `DAI_core` (c22b non-missing). This is the sample used for the pooled three-way moderation specification (Table 3, §4.4) and the dynamic moderation joint F-tests (F1, F2, F3).

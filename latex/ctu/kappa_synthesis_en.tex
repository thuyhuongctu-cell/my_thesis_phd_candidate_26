...
\setstretch{1.15}
\hypertarget{internationalisation-and-firm-performance-in-asia-an-institution-contingent-reconception}{%
\section{Internationalisation and Firm Performance in Asia: An
Institution-Contingent
Reconception}\label{internationalisation-and-firm-performance-in-asia-an-institution-contingent-reconception}}
...
reported as they appear in the dissertation\textquotesingle s results
chapters and are not re-derived here.
\end{quote}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{1-phenomenon-and-motivation}{%
\subsection{1. Phenomenon and
motivation}\label{1-phenomenon-and-motivation}}

For four decades the internationalisation--performance (I--P)
...
Hsu (2003), has been the \textbf{inverted-U}: early internationalisation
...
field has largely treated this instability as residual noise ---
"context matters" --- rather than as a structured, predictable
phenomenon.
...
internationalisation a first-order policy question, not merely an
academic one. Second, an \textbf{institutional spectrum} of unusual
breadth coexists within a single region --- from innovation-driven
...
the learning mechanism at the heart of internationalisation theory,
...
\hypertarget{2-research-gap-and-questions}{%
\subsection{2. Research gap and
questions}\label{2-research-gap-and-questions}}
...
(Marano et al., 2016); it has not shown that institutions govern the
\emph{sign and functional form} of the effect across a continuous
spectrum, nor identified the point at which the inverted-U family ceases
to apply. In parallel, the digitalisation literature has tended to
...
internationalisation affect the performance of Asian firms, and what
...
\tightlist
\item
  \textbf{RQ1.} What is the pooled effect of internationalisation on
...
\hypertarget{3-theoretical-framework-the-cdcm-architecture}{%
\subsection{3. Theoretical framework: the CDCM
architecture}\label{3-theoretical-framework-the-cdcm-architecture}}

The dissertation\textquotesingle s theoretical contribution is organised
by a single integrative architecture, the
\textbf{Country--Digital--Capability Moderation (CDCM)} framework, which
combines four classic theories with a digital-capability lens not as a
mechanical concatenation but as an internally consistent multi-tier
explanation in which each tier accounts for a distinct source of
heterogeneity in the I--P relationship.

\begin{itemize}
\tightlist
\item
  The \textbf{Uppsala internationalisation model} supplies the baseline
...
proposition. The apex of the architecture is the \textbf{Forced
Internationalisation Penalty (FIP)}, a named boundary condition stating
that when all three CDCM tiers are simultaneously minimal, the I--P
relationship loses not only its early-stage benefit but its inverted-U
form, tilting toward a monotone-negative limiting case.

\hypertarget{4-the-compilation-and-how-each-component-earns-its-place}{%
\subsection{4. The compilation and how each component earns its
place}\label{4-the-compilation-and-how-each-component-earns-its-place}}
...
Table 1 traces the \textbf{golden thread} --- the line that runs from
each research question, through the components that answer it and the
...
\toprule\noalign{}
RQ & Component (ICRV) & Hypothesis & Key empirical finding & Theoretical
contribution \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
\emph{Note. Figures are reproduced from the
dissertation\textquotesingle s results chapters (§4.2--§4.9; §5.6.1).
ICRV groups: I = innovation-driven advanced; III = upper-middle; IV =
lower-middle transition; VI = SIDS.}

\hypertarget{5-the-integrated-finding-a-three-zone-morphology}{%
\subsection{5. The integrated finding: a three-zone
morphology}\label{5-the-integrated-finding-a-three-zone-morphology}}

Placed side by side, the full set of ten component studies plus the
supplementary book chapter --- the 17-economy foundational study (P1),
the global meta-analysis (P6), five single-country estimations (P2--P5,
P9), the 50-economy pooled test (P7), two boundary cases (P8 SIDS, P10
Japan), and the governance-tier book chapter --- converge on a single,
falsifiable theoretical claim: \textbf{internationalisation improves
firm performance through a functional form that is itself a function of
institutional position.} The thickness of the institutional cushion
determines whether the I--P relationship is near-linear positive, a
sharp inverted-U, or structurally absent and sign-reversed.
...
\item
  ...
  without being constrained to do so --- an out-of-sample coincidence
  that resists a specification-search reading.
\item
...
This finding elevates the field\textquotesingle s classic claim from
"context matters" (institutions moderate the \emph{magnitude} of the
effect) to \textbf{context-contingency} (institutions moderate the
\emph{sign and form} of the effect), and it identifies explicitly the
point --- the SIDS regime --- at which the entire inverted-U theory
family ceases to apply.

\hypertarget{6-theoretical-contributions}{%
\subsection{6. Theoretical
contributions}\label{6-theoretical-contributions}}
...
\item
  \textbf{The Forced Internationalisation Penalty (FIP).} A newly named
  ...
  a whole family of internationalisation theories that tacitly assume
...
\hypertarget{7-boundary-conditions-and-identification}{%
\subsection{7. Boundary conditions and
identification}\label{7-boundary-conditions-and-identification}}
...
internationalisation measured as \textbf{export intensity} (the
sales-based dimension of the degree of internationalisation; Sullivan,
...
\hypertarget{8-implications}{%
\subsection{8. Implications}\label{8-implications}}
...
inverted-U?" but "at what institutional position does each form obtain?"
For \textbf{practice and policy}, it replaces a one-size-fits-all
prescription with a regime-conditioned policy map: in strong-institution
regimes, remove expansion ceilings and raise absorptive-capacity
ceilings; in transition regimes, identify the optimum and avoid
over-expansion ahead of coordination capacity; in the SIDS regime,
address structural trade-cost and institutional constraints before
expecting internationalisation to improve performance --- because there
internationalisation is a survival burden, not a competitive-advantage
choice.

\hypertarget{9-limitations-and-future-research}{%
\subsection{9. Limitations and future
research}\label{9-limitations-and-future-research}}

The principal limitations are stated as study weaknesses distinct from
the framework\textquotesingle s boundary conditions: the
single-dimension (export-intensity) operationalisation of
internationalisation; the labour-productivity (rather than
multidimensional) performance measure; the repeated-cross-section
design; and the platform-level operationalisation of digital adoption.
Each maps to a future-research direction: multidimensional DOI measures,
multidimensional performance, genuine firm-level panels (the forthcoming
WBES core panel), richer Tier-2 digital measures, and replication of the
three-zone morphology beyond Asia-Pacific.

\hypertarget{10-concluding-statement}{%
\subsection{10. Concluding statement}\label{10-concluding-statement}}

The dissertation\textquotesingle s single new sentence to the field is
this: across the institutional spectrum of Asia and the Pacific, the
form of the internationalisation--performance relationship is itself
governed by institutions, such that the inverted-U is one
regime-specific case within a continuum that runs from near-linear
reward at the upper bound to structural penalty at the lower bound ---
and the dissertation identifies, and names, the point at which the
inverted-U no longer applies.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\emph{Citation note: all references appear in the
dissertation\textquotesingle s APA-7 reference list
(\texttt{thesis/04\_references\_apa7.md}). This synthesis introduces no
claims or figures beyond those in the Vietnamese dissertation chapters.}

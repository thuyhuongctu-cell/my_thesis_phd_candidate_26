...
## Phân biệt em dash văn xuôi với em dash cấu trúc (BẮT BUỘC)

Bài học thực tế: trong manuscript/luận án, ĐA SỐ em dash (—) nằm ở thành phần **cấu trúc**, không phải câu văn. Gỡ bừa sẽ phá định dạng. Trước khi đổi, **loại trừ** các dòng:
- Tiêu đề Markdown (bắt đầu bằng `#`)
- Hàng bảng (chứa `|`) — kể cả nhãn ô như `I — Advanced`, `Advanced — innovation-driven`
- Chú thích/nguồn hình & bảng (bắt đầu `![`, `*Hình`, `*Bảng`, `*Hộp`, `*Ghi chú`, `*Nguồn`, hoặc chứa `](figures`)
- Nhãn trích dẫn nội bộ (vd `(2026 — VEFR)`, `Tác giả, năm — venue`)

**Chỉ đổi em dash trong câu văn.** TUYỆT ĐỐI không đụng en dash `–` (U+2013) — nó đúng trong khoảng số (2009–2025), cặp quốc gia, và từ ghép (de jure–de facto, I–P).

Tìm nhanh em dash văn xuôi:
```bash
grep -nE '—' FILE | grep -vE '^\s*[0-9]+:\s*(#|\||!\[|\*Hình|\*Bảng|\*Hộp|\*Ghi chú|\*Nguồn)' | grep -vE '\]\(figures'
```

## Bước 5b: Verify toàn vẹn dữ liệu (BẮT BUỘC sau khi sửa)

Humanize chỉ được đổi dấu câu/chữ, KHÔNG được đổi số liệu hay en dash. Kiểm chứng so với bản gốc (HEAD):
```bash
# chữ số phải y hệt
diff <(git show HEAD:FILE | grep -oE '[0-9]') <(grep -oE '[0-9]' FILE)
# số lượng en dash phải y hệt
[ "$(git show HEAD:FILE | grep -o '–' | wc -l)" = "$(grep -o '–' FILE | wc -l)" ] && echo OK
```
Nếu chữ số hoặc en dash lệch ⇒ đã lỡ đụng dữ liệu, phải sửa lại trước khi commit.

## Khi uỷ thác cho subagent

Nếu giao việc humanize cho subagent (file lớn / nhiều file), trong prompt phải:
- Liệt kê **preserve-list tuyệt đối**: số/thập phân, thống kê, p-value, §-ref, trích dẫn, LaTeX, **bold**/*italic*, cấu trúc bảng, en dash.
- Nêu rõ quy tắc loại trừ em dash cấu trúc ở trên.
- Yêu cầu báo cáo lại: đếm em dash trước/sau, xác nhận en dash & chữ số không đổi.
- **Không commit khi agent đang sửa**; chờ agent xong, chạy verify (Bước 5b), rồi mới commit.

## Checklist cuối cùng

Trước khi hoàn tất:
- [ ] Chỉ đổi em dash văn xuôi; em dash cấu trúc (bảng/tiêu đề/caption/trích dẫn) giữ nguyên
- [ ] En dash `–` và mọi chữ số không đổi (đã chạy verify Bước 5b)
